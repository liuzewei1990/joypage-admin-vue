...
<template>
  ...
  <el-select :class="{ 'bc-select': true, 'auto-w': isAutoWidth }" ref="el-select" v-model="selectVaule" :loading="loading" @change="$emit('change', $event)" @clear="$emit('clickItem', {})" @blur="blur" @focus="focus" :value-key="itemNameKey" @visible-change="visibleChange($event)" :remote-method="remoteMethod" :disabled="disabled" :placeholder="placeholder" :filterable="filterable" :remote="remote" :clearable="clearable" :multiple="multiple">
    <!-- 数据列表 -->
    <el-option v-for="item in dataset.list" :key="item[itemValueKey]" :value="item[itemValueKey]" :label="item[itemNameKey]" @click.native="$emit('clickItem', item)"></el-option>
