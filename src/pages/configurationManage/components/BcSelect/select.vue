...
<template>
  ...
  <el-select :class="{'bc-select':true,'auto-w':isAutoWidth}" ref="el-select" v-model="selectVaule" :loading="loading" @change="$emit('change',$event)" @blur="blur" @focus="focus" value-key="name" @visible-change="visibleChange($event)" :remote-method="remoteMethod" :disabled="disabled" :placeholder="placeholder" :filterable="filterable" :remote="remote" :clearable="clearable" :multiple="multiple">
