    ...
    <div class="prdSeries">
                ...
                <el-form :inline="true" :size="size" :model="topCheckData" ref="topData">
                    ...
                    <el-form-item label="所属模块：" prop="modularId">
                        <el-select v-model="topCheckData.modularId" placeholder="请选择系统" clearable>
                            <el-option v-for="item in moduleList" :key="item.name" :label="item.modularName" :value="item.id"> </el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
            </div>
        ...
        <el-dialog :title="!dataForm.id ? '新增' : dialogTitle" width="40%" :visible.sync="dialogVisible" :before-close="closeDialog">
            <el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size" label-position="left" class="addForm">
                ...
                <el-form-item label="所属模块：" prop="modularId">
                    <el-select v-model="dataForm.modularId" placeholder="请选择" filterable clearable @change="changeContentType">
                        <el-option v-for="item in moduleList" :key="item.value" :label="item.modularName" :value="item.id"> </el-option>
                    </el-select>
                </el-form-item>
        ...
        data() {
            return {
                ...
                formRules: {
                    ...
                    picUrl: [{ required: true, message: "请添加图片", trigger: "change" }],
                    modularId: [{ required: true, message: "请选择所属模块", trigger: "change" }],
