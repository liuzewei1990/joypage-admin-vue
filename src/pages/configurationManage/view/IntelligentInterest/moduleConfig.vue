...
<!-- 配置管理-智能投顾配置-模块配置 -->
  <div class="prdSeries">
    <CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="moduleList" style="width: 100%" stripe border="true"
	...
	methods: {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						updateModuleSave(params).then((resData) => {
		...
		handleDelete (index, row) {
            ...
            this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
                ...
                getModuleDelete(params).then(resData => {
