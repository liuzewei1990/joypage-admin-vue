...
<!-- 配置管理-公共模块-首页标签筛选接口 -->
  <div class="prdSeries">
	...
	<div class="topBox">
		<el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
            ...
            <el-form-item label="标签名称：" prop="lableValue">
				<el-select v-model="topCheckData.lableValue" placeholder="请选择" filterable clearable
				@change="lableNameListSelect">
					<el-option v-for="item in allSelectData.lableNameList"
					:key="item.DIC_VALUE" :label="item.DIC_NAME" :value="item.DIC_VALUE">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="功能开关：" prop="switchStr">
				<el-switch v-model="topCheckData.switchStr" active-color="#409EFF" inactive-color="#ff4949" 
				active-value="1" inactive-value="0" active-text="YES" inactive-text="NO" @change="getuUpdSwitchStr()"></el-switch>
			</el-form-item>
    ...
    <div class="table">
		...
		<!-- 表格内容栏 -->
		<el-table :height="500" size="mini" :data="lableList" style="width: 100%" stripe border="true"
	...
	<el-dialog :title="!dataForm.ID ? '新增' : '修改'" width="60%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
            ...
            <el-form-item label="标签名称：" prop="lableName">
				<el-select v-model="dataForm.lableValue" placeholder="请选择" filterable clearable
				@change="lableNameListSelect">
					<el-option v-for="item in allSelectData.lableNameList"
					:key="item.DIC_VALUE" :label="item.DIC_NAME" :value="item.DIC_VALUE">
					</el-option>
            ...
            <el-form-item label="是否启用：" prop="isOk">
				<el-switch v-model="dataForm.isOk" active-color="#409EFF" inactive-color="#ff4949" 
				active-value="1" inactive-value="0" active-text="YES" inactive-text="NO" @change="getuUpdSwitchStr()"></el-switch>
			</el-form-item>
		</el-form>
...
export default {
	...
	data() {
		return {
			...
			formRules: { //弹框校验规则
				orderNum:[
					{ required: true, message: '请选择展示位编码', trigger: 'blur' },
				],
				lableName:[
	...
	methods: {
		...
		getQueryLableSelectList: function(type) {
			...
			queryLableSelectList(params).then(res => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						createLableSelect(params).then((res) => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updLableSelect(params).then((res) => {
		...
		handleDelete (index, row) {
			...
			if(row.fundStatus == '0'){
				...
				this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
					...
					deleteLableSelect(params).then(res => {
