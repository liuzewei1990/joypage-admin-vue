                ...
                <el-form :inline="true" :size="size" :model="topCheckData" ref="topData">
                    ...
                    <el-form-item label="系统：" prop="systemTypes">
                        <el-select v-model="topCheckData.systemTypes" placeholder="请选择系统" clearable>
                            <el-option v-for="item in allSelectData.systemTypeList" :key="item.name" :label="item.name" :value="item.value"> </el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            handleEdit: function (row, index) {
                ...
                this.dataForm.orgId = {
                    //组件回来的orgId为对象--解决自动匹配机构和类型时因列表中没有该项导致回显失败的问题
                    id: row.orgId,
                    name: row.orgName
                };
            ...
            handleExport() {
                ...
                a.href =
                    config.baseUrlHost +
                    "/cInclusiveAccountProfit/exportFile?" +
                    "orgId=" +
                    this.topCheckData.orgId +
                    "&keywords=" +
                    this.topCheckData.keywords +
                    "&systemTypes=" +
                    this.topCheckData.systemTypes;
                a.click();
            }
        }
    };
