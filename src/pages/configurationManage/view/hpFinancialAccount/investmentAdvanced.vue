<template>
  ...
  <div class="prdSeries">
    ...
    <CommonTable :isFluid="false" @height="tableHeight=$event">
      ...
      <!-- 表格内容 -->
      <el-table :height="tableHeight" size="mini" :data="tableData.tableDataList" style="width: 100%" stripe border="true" @selection-change="selectionChange" v-loading="loading" element-loading-text="loading...">
      ...
      <!-- 分页 -->
      <el-pagination slot="footer" style="float:right;" :page-sizes="[10, 20, 30, 40]" :page-size="topCheckData.pageSize" @size-change="handleSizeChange" @current-change="currentChange" :current-page="topCheckData.currentPage" :pager-count="7" small layout="total, sizes, prev, pager, next, jumper" :total="tableData.totalCount"></el-pagination>
    </CommonTable>
    ...
    <el-dialog :title="dialogTitle" width="600px" :visible.sync="dialogVisible" :before-close="closeDialog" @open="dialogOpen">
      <el-form :model="addFromData" label-width="auto" :rules="formRules" ref="dataForm" :size="size" label-position="left" class="addForm">
        ...
        <el-form-item label="选择机构" filterable prop="orgId">
          <BcSelect :apiType="2" ref="OrgBcSelect" v-if="dialogVisible" v-model="addFromData.orgId" @change="orgIdOnChange" :filterable="true" :remote="true" :disabled="dialogTitle==='编辑'" placeholderText="请输入/选择机构"></BcSelect>
        </el-form-item>

        <el-form-item label="选择产品类型" filterable prop="prdTypeId">
          <BcSelect :apiType="3" ref="PrdtypeBcSelect" v-if="dialogVisible" v-model="addFromData.prdTypeId" @change="prdTypeOnChange" :filterable="true" :remote="false" :pageSize="50" :disabled="dialogTitle==='编辑'" placeholderText="请输入/选择产品类型）"></BcSelect>
        </el-form-item>

        <el-form-item label="选择产品" filterable prop="prdIndexId">
          <BcSelect :apiType="1" ref="PrdBcSelect" v-if="dialogVisible" v-model="addFromData.prdIndexId" @change="prdOnChange" @visibleChange="prdBcSelectVisibleChange" :filterable="true" :remote="true" :disabled="dialogTitle==='编辑'" placeholderText="请选择产品，支持模糊查询（不填代表全部产品）" :tooltipContent="`当前机构暂无产品`" :apiAuto="true"></BcSelect>
        </el-form-item>

        <el-form-item label="生效时间" prop="dateTime">
          <el-date-picker v-model="addFromData.dateTime" type="datetimerange" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
        </el-form-item>
