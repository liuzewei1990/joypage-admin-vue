...
<template>
  ...
  <el-tooltip class="item" :disabled="tooltipDisabled" effect="dark" :content="tooltipContent" :open-delay="1500" placement="right">
    <el-select class="bc-select" ref="el-select" v-model="selectVaule" @change="$emit('change',$event)" @blur="blur" @focus="focus"
    value-key="name" @visible-change="$emit('visibleChange',$event)" :remote-method="remoteMethod" :filter-method="filterMethod"
    :disabled="disabled" :placeholder="placeholder" :filterable="filterable" :remote="remote" :clearable="clearable">
