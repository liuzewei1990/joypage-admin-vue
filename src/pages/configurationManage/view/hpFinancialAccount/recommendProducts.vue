...
<!-- 配置管理--普惠理财账-推荐产品 -->
  <div class="prdSeries">
    <CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<div slot="search">
			<el-form :inline="true" :size="size"  :model="topCheckData" ref='topData'>
				...
				<el-form-item label="可见用户：" prop="advVisibleUesr">
                    <BcSelectStr v-model="topCheckData.advVisibleUesr" @GetApiData="SetAvailableUserList($event)" :filterable="true" :remote="true" :placeholder="'请选择'" :clearable='true'></BcSelectStr>
				</el-form-item>
			</el-form>
		</div>
		<!-- 操作按钮区 -->
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="recommendedList" style="width: 100%" stripe border="true"
	...
	methods: {
		...
		handleDelete (index, row) {
            ...
            this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
                ...
                getRecommendedDelete(params).then(resData => {
