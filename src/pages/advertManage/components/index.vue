...
<template>
  ...
  <el-tooltip class="item" :disabled="tooltipDisabled" effect="dark" :content="tooltipContent" :open-delay="1500" placement="right">
    <el-select class="bc-select" ref="el-select" v-model="selectVaule" @change="$emit('change',$event)" @blur="blur" @focus="focus" value-key="name" @visible-change="$emit('visibleChange',$event)" :remote-method="remoteMethod" :disabled="disabled" :placeholder="placeholder" :filterable="filterable" :remote="remote" :clearable="clearable" :multiple="multiple">
      <!-- 特殊处理，隐藏域，解决回显的数据在list列表中不存在，导致回显失败的问题 -->
      <el-option class="default-option-hide" v-show="false" :key="selectVaule.id" :value="selectVaule" :label="selectVaule.name"></el-option>
      <!-- 数据列表 -->
...
//   },
  ...
  methods: {
	...
	handleSizeChange(val){
		this.datatypeParam.pageSize = val
		this.getCommmonSelectList();
	},
