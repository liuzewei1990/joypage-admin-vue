      ...
      <el-form-item v-if='advType==1' label="图片描述：" prop='advImgMemo'>
        <el-input v-model="form.advImgMemo"></el-input>
            ...
            <el-form-item v-if="advType == 2" label="弹框图片：" prop="advImgUrl">
                <el-upload :model="form.advImgUrl" class="upload-demo" action="customize" :http-request="uploadCheckImg" :show-file-list="false" :accept="acceptType">
                    <img v-if="imageUrl" :src="imageUrl" class="avatar" />
                    <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                </el-upload>
            </el-form-item>
            ...
            <el-form-item label="投放渠道：" prop="appFlag">
                <el-select clearable v-model="form.appFlag" multiple placeholder="请选择">
                    <el-option v-for="item in advChannelList" :key="item.value" :label="item.name" :value="item.value"> </el-option>
                </el-select>
            </el-form-item>
