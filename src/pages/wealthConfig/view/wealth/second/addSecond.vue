    ...
    <div class="editAdvertPosit" v-loading="loadingCon">
        <el-form :size="size" ref="form" :rules="rules" :model="form" label-width="140px">
            ...
            <el-form-item label="生效时间：" prop="effectiveDatetimeStr">
                <el-date-picker v-model="form.effectiveDatetimeStr" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期时间"> </el-date-picker>
            </el-form-item>
                    ...
                    <li v-for="(item, index) in wealthTwoMenuImgList" :key="index">
                        <div class="imgCon">
                            <el-form-item :label="'图片' + (index - -1) + '：'" label-width="80px">
                                <div @click="uploadonprogress(index)">
                                    <el-upload :model="item.imgUrl" accept="image/*" class="upload-demo upload-demo1" action="customize" :http-request="uploadCheckImg" :show-file-list="false">
                        ...
                        <div>
                            ...
                            <el-form-item label="请选择：" label-width="100px" style="position: relative">
                                ...
                                <el-select
                                    @focus="clickContent(index, item.contentName)"
                                    ref="scrollbar"
                                    no-match-text=""
                                    class="scrollSelect"
                                    @change="changeContent(index, item.contentName)"
                                    clearable
                                    v-else
                                    v-model="item.contentName"
                                    :filterable="item.contentType == 3"
                                    remote
                                    @blur="blurFun(index, item.contentName)"
                                    :loading="loading"
                                    :remote-method="searchPro"
                                    placeholder="">
                                    <div v-if="item.contentType == 3">
                                        <el-option
                                            :class="item.contentType == 5 ? 'zixunClass' : ''"
                                            v-for="(item1, index1) in contentList"
                                            :key="index1"
                                            :label="item1.name + '-' + item1.value"
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    effectiveDatetimeStr: [{ required: true, message: "请选择生效时间", trigger: "change" }],
                    title: [{ required: true, message: "请输入标题", trigger: "change" }],
                    templateType: [{ required: true, validator: templateTypeVal, trigger: "change" }],
                    systemTypes: [{ required: true, message: "请选择生效设备", trigger: "change" }],
