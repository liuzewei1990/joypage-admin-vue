    ...
    <div class="addBank" v-loading="loading">
        ...
        <el-form :size="size" ref="form" :rules="rules" :model="form" label-width="160px">
            <el-form-item label="银行" prop="orgId">
                <el-select :disabled="$route.query.name == '修改'" v-model="form.orgId" @change="checkBank" filterable clearable placeholder="请选择">
                    <span class="no">平台已配置银行</span>
                    <el-option v-for="item in platformConfigExistArr" :key="item.id" :label="item.orgName" :value="item.id"> </el-option>
                    <span class="ok">平台未配置银行</span>
                    <el-option v-for="item in platformConfigExistArrNO" :key="item.id" disabled :label="item.orgName" :value="item.id"> </el-option>
                </el-select>
                <div class="tipTxtClass">{{ tipTxt }}</div>
            </el-form-item>
            <el-form-item label="银行产品" prop="prdIndexId">
                <el-select :disabled="form.orgId == '' || $route.query.name == '修改'" v-model="form.prdIndexId" filterable clearable placeholder="请选择">
                    <el-option v-for="item in bankProList" :key="item.id" :label="item.prdName + '-' + item.id" :value="item.id"> </el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="生效时间" prop="times">
                <el-date-picker v-model="form.times" type="datetimerange" range-separator="至" value-format="yyyy-MM-dd HH:mm:ss" start-placeholder="开始日期" end-placeholder="结束日期">
                </el-date-picker>
            </el-form-item>
...
<script>
    import configMsg from "../../../component/userMarketMange/product/configMsg";
    import alertPop from "../../../component/userMarketMange/alertPOP";
    import { BankPrdselectOne, BankPrdsave, org, bankPrd } from "../../../apis/index";
    export default {
        ...
        data() {
            ...
            var configMsgVal = (rule, value, callback) => {
                ...
                if (this.configMsgData.oneLevelUserCondition == undefined) {
                    ...
                    if (this.configMsgData.oneLevelUserCondition.filter((a) => a.dataType != "3" && a.conditionVisible != "").length == 0) {
                        isOk = "请配置限制条件";
                    }
                    if (this.configMsgData.oneLevelUserCondition.filter((a) => a.dataType != "3")[0].conditionVisible == "") {
                        isOk = "一级条件的第一项必填";
                    }
                    this.configMsgData.oneLevelUserCondition
                        .filter((a) => a.dataType != "3" && a.conditionVisible != "")
                        .forEach((i, index) => {
                            let length = this.configMsgData.oneLevelUserCondition.filter((a) => a.dataType != "3" && a.conditionVisible != "").length;
                            if (i.condConnSymbol == "" || (i.condConnSymbol == null && index < length - 1)) {
                                isOk = "请选择二级条件之间的关系";
                            }
                        });
                    this.configMsgData.twoLevelUserCondition
                        .filter((a) => a.dataType != "3" && a.conditionVisible != "")
                        .forEach((i, index) => {
                            let length = this.configMsgData.twoLevelUserCondition.filter((a) => a.dataType != "3" && a.conditionVisible != "").length;
                            if (i.condConnSymbol == "" || (i.condConnSymbol == null && index < length - 1)) {
                                isOk = "请选择二级条件之间的关系";
                            }
                        });
            ...
            return {
                ...
                rules: {
                    times: [{ required: true, validator: timesVl, trigger: "change" }],
                    orgId: [{ required: true, message: "请选择银行", trigger: "change" }],
                    prdIndexId: [{ required: true, message: "请选择银行产品", trigger: "change" }],
                    configMsg: [{ required: true, validator: configMsgVal, trigger: "change" }]
                },
                StarttimeString: 0,
                EndtimeString: 0,
                oldMsg: "",
        ...
        computed: {
            ...
            title() {
                return `${this.$route.query.name}银行配置`;
            }
            // configMsgData(){
        ...
        created() {
            this.$route.meta.title = `${this.$route.query.name}银行配置`;
        ...
        mounted() {
            //  console.log('copy',this.$route.query.copy)
        },
        methods: {
            // 选择银行
            checkBank(value) {
            ...
            sumFun() {
                ...
                this.configMsgData.oneLevelUserCondition
                    ...
                    .forEach((i) => {
                        if (
                            this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.conditionVisible != "").length == 1 &&
                            this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.conditionVisible != "")[0].conditionVisible == ""
                        ) {
                            this.$refs.configMsgData.disabledRadio2 = false;
                        } else {
                            this.configMsgData.twoLevelUserCondition
                                .filter((b) => b.dataType != "3" && b.conditionVisible != "")
                                .forEach((j) => {
                                    // debugger
                                    if (i.conditionVisible != j.conditionVisible) {
                                        isSame = 1; // 配置的可见状态不相同
                                    }
                                    if (this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.conditionVisible != "").length > 1) {
                                        let conditionVisible = this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.conditionVisible != "")[0].conditionVisible;
                                        if (j.conditionVisible != conditionVisible) {
                                            twoisSame = 1;
                                        }
                                        console.log("twoisSame", twoisSame);
                                        if (twoisSame == 1) {
                                            this.configMsgData.twoLevelUserCondition
                                                .filter((i) => i.dataType != "3" && i.conditionVisible != "")
                                                .forEach((k) => {
                                                    k.condConnSymbol = 2;
                                                    console.log("k::", k);
                                                });
                        ...
                        if (this.configMsgData.oneLevelUserCondition.filter((i) => i.dataType != "3" && i.conditionVisible != "").length > 1) {
                            let conditionVisible = this.configMsgData.oneLevelUserCondition.filter((i) => i.dataType != "3" && i.conditionVisible != "")[0].conditionVisible;
                            if (i.conditionVisible != conditionVisible) {
                                oneisSame = 1;
                            }
                            if (oneisSame == 1) {
                                this.configMsgData.oneLevelUserCondition
            ...
            save() {
                let arr1 = [];
                let arr2 = [];
                console.log("configMsgData::", this.configMsgData);
                this.$refs["form"].validate((valid) => {
                    if (valid) {
                        ...
                        // console.log('提交的参数',params.datatypeParam)
                        // return
                        BankPrdsave(params)
                            .then((res) => {
                                console.log("res", res);
