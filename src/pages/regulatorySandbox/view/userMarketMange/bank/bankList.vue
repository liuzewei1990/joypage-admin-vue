<template>
    ...
    <div class="bankList">
        <el-form :size="size" class="searchForm" :model="searchForm" ref="searchForm">
            <div class="searchDiv">
                <el-form-item label="关键字：" prop="keywords">
                    <el-input ref="keywords" :disabled="searchForm.orgIds.length > 0" v-model="searchForm.keywords" type="text" clearable placeholder="请输入关键字"></el-input>
                </el-form-item>
                <el-form-item label="银行：" prop="orgIds">
                    <el-select ref="orgIds" :disabled="searchForm.keywords != ''" v-model="searchForm.orgIds" filterable clearable multiple placeholder="请选择">
        ...
        <div class="addBtn">
            <el-button :size="size" icon="el-icon-plus" type="primary" @click="add">新增银行配置</el-button>
                        ...
                        <div class="contentDiv" v-for="(item, index) in scope.row.oneLevelUserCondition" :key="index">
                            <div class="content">
                                ...
                                <ul>
                                    ...
                                    <div
                                        v-if="
                                            (item.ageConnSymbol == '' || item.ageConnSymbol == null) &&
                                            (item.userStartAge == '' || item.userStartAge == null) &&
                                            (item.userEndAge == '' || item.userEndAge == null)
                                        "></div>
                                    <div v-else>
                                        <span>年龄：</span>
                                        <li>
                                            {{ item.ageStartSymbol }}{{ item.userStartAge }}{{ item.userStartAge == "" || item.userStartAge == null ? "" : "岁"
                                            }}{{ item.ageConnSymbol == 1 ? "且" : "" }}{{ item.ageConnSymbol == 2 ? "或" : "" }}{{ item.ageEndSymbol }}{{ item.userEndAge
                                            }}{{ item.userEndAge == "" || item.userEndAge == null ? "" : "岁" }}
                                        </li>
                                    </div>
                                    <!-- 登陆状态 -->
                                    <div
                                        v-if="
                                            (item.userRegStatus != '' && item.userRegStatus != null) ||
                                            (item.realNameStatus != '' && item.realNameStatus != null) ||
                                            (item.openAccountStatus != '' && item.openAccountStatus != null) ||
                                            (item.investmentStatus != '' && item.investmentStatus != null) ||
                                            (item.holdSharesStatus != '' && item.holdSharesStatus != null)
                                        ">
                                        <span>比财：</span>
                                        <li v-if="item.userRegStatus == '0'">未登陆</li>
                                        <!-- 登陆时间 -->
                                        <li v-if="(item.regConnectionSymbol != '' && item.regConnectionSymbol != null) || item.userRegStatus == 1">
                                            已登陆；{{ item.regStartSymbol }}{{ item.regStartTime }} {{ item.regConnectionSymbol == 1 ? "且" : "" }}{{ item.regConnectionSymbol == 2 ? "或" : "" }}
                                    ...
                                    <div
                                        v-if="
                                            (item.holdSharesPeakValueStartAmount != '' && item.holdSharesPeakValueStartAmount != null) ||
                                            (item.holdSharesPeakValueEndAmount != '' && item.holdSharesPeakValueEndAmount != null) ||
                                            (item.localBankHoldPeakValueStartAmount != '' && item.localBankHoldPeakValueStartAmount != null) ||
                                            (item.localBankHoldPeakValueEndAmount != '' && item.localBankHoldPeakValueEndAmount != null)
                                        ">
                                        <span>持仓峰值金额：</span>
                                        <li
                                            v-if="
                                                ...
                                                (item.holdSharesPeakValueEndAmount != '' && item.holdSharesPeakValueEndAmount != null)
                                            ">
                                            比财：{{ item.holdSharesPeakValueStartSymbol }}{{ item.holdSharesPeakValueStartAmount }} {{ item.holdSharesPeakValueConnSymbol == 1 ? "且" : ""
                                            }}{{ item.holdSharesPeakValueConnSymbol == 2 ? "或" : "" }} {{ item.holdSharesPeakValueEndSymbol }}{{ item.holdSharesPeakValueEndAmount }}
                                        </li>
                                        ...
                                        <li
                                            v-if="
                                                ...
                                                (item.localBankHoldPeakValueEndAmount != '' && item.localBankHoldPeakValueEndAmount != null)
                                            ">
                                            本银行：{{ item.localBankHoldPeakValueStartSymbol }}{{ item.localBankHoldPeakValueStartAmount }}
                                            <span v-if="item.localBankHoldPeakValueConnSymbol == 1">且</span>
                                            <span v-if="item.localBankHoldPeakValueConnSymbol == 2">或</span>
                                            {{ item.localBankHoldPeakValueEndSymbol }}{{ item.localBankHoldPeakValueEndAmount }}
                                        </li>
                                    </div>
                                    ...
                                    <div
                                        v-if="
                                            (item.localBankOpenAccountStatus != '' && item.localBankOpenAccountStatus != null) ||
                                            (item.localBankInvestmentStatus != '' && item.localBankInvestmentStatus != null) ||
                                            (item.localBankHoldSharsStatus != '' && item.localBankHoldSharsStatus != null)
                                        ">
                                        <span>本银行：</span>
                                        <li v-if="item.localBankOpenAccountStatus != '' && item.localBankOpenAccountStatus != null">
                                            {{ item.localBankOpenAccountStatus == "1" ? "已开户" : "未开户" }}
                                        </li>
                                        <!-- 本银行的投资状态1投0未投 -->
...
<script>
    import { BankPrdpageQuery, BankPrdcheckExist, org, BankPrddelete, BankPrddeleteBatch } from "../../../apis/index";
    export default {
        data() {
            return {
                contentList: [],
        ...
        methods: {
            ...
            goto() {
                ...
                let tabObj = {
                    href: "/omp/static/html/regulatorySandbox/#/sandboxExternalLink/bankList",
                    icon: null,
                    id: "805",
                    pid: pidNew,
                    title: "外链管理-银行"
