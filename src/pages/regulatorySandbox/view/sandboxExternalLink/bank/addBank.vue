    ...
    <div class="addBank" v-loading="loading">
        ...
        <el-form :size="size" ref="form" :rules="rules" :model="form" label-width="160px">
            <el-form-item label="银行" prop="orgId">
                <el-select :disabled="$route.query.name == '修改'" @change="checkOrg" v-model="form.orgId" filterable clearable placeholder="请选择">
...
<script>
    import configMsg from "../../../component/sandboxExternalLink/bank/configMsg";
    import alertPop from "../../../component/sandboxExternalLink/alertPOP";
    import { ExternalLinkBankPrdselectOne, ExternalLinkBankPrdsave, org, BankPrdcheckExist } from "../../../apis/index";
    export default {
            ...
            var configMsgVal = (rule, value, callback) => {
                ...
                if (this.configMsgData.oneLevelUserCondition == undefined) {
                    ...
                    this.configMsgData.oneLevelUserCondition
                        .filter((a) => a.dataType != "3" && a.buyStatus != "")
                        .forEach((i, index) => {
        ...
        computed: {
            ...
            title() {
                return `${this.$route.query.name}银行外链配置`;
            }
        },

        created() {
            this.$route.meta.title = `${this.$route.query.name}银行外链配置`;
        ...
        methods: {
            ...
            sumFun() {
                ...
                this.configMsgData.oneLevelUserCondition
                    ...
                    .forEach((i) => {
                        if (
                            this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.buyStatus != "").length == 1 &&
                            this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.buyStatus != "")[0].buyStatus == ""
                        ) {
                            this.$refs.configMsgData.disabledRadio2 = false;
                        } else {
                            this.configMsgData.twoLevelUserCondition
                                .filter((b) => b.dataType != "3" && b.buyStatus != "")
                                .forEach((j) => {
                                    // debugger
                                    ...
                                    if (this.configMsgData.twoLevelUserCondition.filter((i) => i.dataType != "3" && i.buyStatus != "").length > 1) {
                                        ...
                                        if (twoisSame == 1) {
                                            this.configMsgData.twoLevelUserCondition
                                                .filter((i) => i.dataType != "3" && i.buyStatus != "")
                                                .forEach((k) => {
                                                    k.condConnSymbol = 2;
                                                    console.log("k::", k);
                                                });
            ...
            BankPrdselectOneFun() {
                ...
                ExternalLinkBankPrdselectOne(params)
                    .then((res) => {
                        this.loading = false;
                        if (res.headerModel.code == 0) {
            ...
            cancle() {
                ...
                if (this.oldMsg != JSON.stringify(this.form) || newMsgConfig != this.oldMsgConfig) {
                    this.$confirm("还要继续吗？", "现在取消已编辑的内容将无法保存", {
                        ...
                        .then(() => {
                            this.$router.push("/sandboxExternalLink/bankList");
                        })
                        .catch(() => {});
                } else {
                    this.$router.push("/sandboxExternalLink/bankList");
            ...
            save() {
                ...
                this.$refs["form"].validate((valid) => {
                    if (valid) {
                        ...
                        // return
                        ExternalLinkBankPrdsave(params)
                            .then((res) => {
                                console.log("res", res);
