...
<!-- 监管沙盒外链  银行管理  -->
  <div class='bankList'>
    <el-form :size='size' class='searchForm' :model='searchForm' ref='searchForm'>
      <div class='searchDiv'>
        <el-form-item label='关键字：' prop="keywords">
          <el-input :disabled='searchForm.orgIds.length>0' v-model='searchForm.keywords' type='text' clearable placeholder="请输入关键字"></el-input>
        </el-form-item>
        <el-form-item label='银行：' prop="orgIds">
          <el-select :disabled='searchForm.keywords!=""' v-model="searchForm.orgIds" filterable clearable multiple placeholder="请选择">
            <el-option
            v-for="item in orgs"
            :key="item.id"
            :label="item.orgName"
    ...
    <div class='addBtn'>
      <el-button :size='size' icon='el-icon-plus' type="primary" @click='add'>新增银行外链配置</el-button>
          ...
          <el-table-column prop=""  header-align="center" align="left">
            <template slot-scope="scope">
              <div class='itemDiv' style='position:relative'>
                ...
                <div>监管提示文案：{{scope.row.buyPromptText}}</div>
                  ...
                  <div class='content'>
                    ...
                    <ul>
                      ...
                      <div v-if='item.userRegStatus!=""&&item.userRegStatus!=null'>
                        <span>注册状态与时间：</span>
                        <!-- <li v-if='item.userRegStatus!=""&&item.userRegStatus!=null'>{{item.userRegStatus=='1'?"已注册":"未注册"}}</li> -->
                        <!-- 注册时间 -->
                        <li v-if='(item.regConnectionSymbol!=""&&item.regConnectionSymbol!=null)'>已注册；{{item.regStartSymbol}}{{item.regStartTime}}{{item.regConnectionSymbol==1?"且":""}}{{item.regConnectionSymbol==2?"或":""}}{{item.regEndSymbol}}{{item.regEndTime}}</li>
                        <li v-else-if='item.regStartTime!=""&&item.regStartTime!=null'>已注册；{{item.regStartSymbol}}{{item.regStartTime}}</li>
                        <li v-else-if='item.regEndTime!=""&&item.regEndTime!=null'>已注册；{{item.regEndSymbol}}{{item.regEndTime}}</li>
                      </div>
...
<script>
import {ExternalLinkBankPrdpageQuery,org,ExternalLinkDelete,ExternalLinkBankPrddeleteBatch} from '../../../apis/index'
export default {
  data () {
    return {
      contentList:[],
  ...
  methods: {
    ...
    goto () {
      ...
      let tabObj = {
        href: "/omp/static/html/regulatorySandbox/#/bank/bankList",
        icon: null,
        id: "803",
        title: "用户营销管理-银行",
        pid:pidNew
