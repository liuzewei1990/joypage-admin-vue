            ...
            <template slot-scope="scope">
              <div class='itemDiv' style='position:relative'>
                ...
                <div class='contentDiv' v-for='(item,index) in scope.row.oneLevelUserCondition' :key='index'>
                    ...
                    <div class="title" :class='item.buyStatus=="0"?"noVisib":""'>{{item.buyStatus=='0'?"不可购买":"可购买"}}条件</div>
                     <ul>
                      ...
                      <div v-if='item.userRegStatus!=""&&item.userRegStatus!=null'>
                        ...
                        <!-- 注册时间 -->
                        <li v-if='(item.regConnectionSymbol!=""&&item.regConnectionSymbol!=null)||item.userRegStatus==1'>已注册；{{item.regStartSymbol}}{{item.regStartTime}}{{item.regConnectionSymbol==1?"且":""}}{{item.regConnectionSymbol==2?"或":""}}{{item.regEndSymbol}}{{item.regEndTime}}</li>
                  ...
                  <div class='HUO' v-if='index!=scope.row.oneLevelUserCondition.length-1'>
                    {{item.condConnSymbol==1?"且":""}}{{item.condConnSymbol==2?"或":""}}
                  </div>
                </div>
                ...
                <div class='contentDiv' v-for='(item,index) in scope.row.twoLevelUserCondition' :key='index'>
                  ...
                  <div class='HUO' v-if='index!=scope.row.twoLevelUserCondition.length-1'>
                    {{item.condConnSymbol==1?"且":""}}{{item.condConnSymbol==2?"或":""}}
                  </div>
                </div>
