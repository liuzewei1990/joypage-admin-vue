...
<!-- 黑名单 手机号管理  -->
  <div class='platformConfig'>
    <el-form :size='size' class='searchForm' :model='searchForm' ref='searchForm'>
      <div class='inputDiv'>
        ...
        <el-form-item label='产品' prop="prdIndexIds">
            <el-select  :disabled='searchForm.orgIds.length>0' multiple no-match-text='' class='scrollSelect' @change='changeContent' clearable  v-model="searchForm.prdIndexIds" filterable remote :remote-method="searchPro" placeholder="请选择">
            <div>
              <el-option
    ...
    <el-table :data="contentList" style="width: 100%" border="true" empty-text='-' :loading='tabLoading' @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55"></el-table-column>
      <el-table-column prop="whiteUserCode" width="250" label="手机号白名单编码" header-align="center" align="center"></el-table-column>
      <el-table-column prop="phoneNum" label="手机号"  width="140" header-align="center" align="center"></el-table-column>
      <el-table-column prop="peopleName" label="人员身份" header-align="center" align="center"></el-table-column>
      <el-table-column prop="internalAppFlagsName" label="开放渠道" header-align="center" align="center">
        <template slot-scope="scope" > 
          <span v-if='scope.row.internalAppFlagsName==""||scope.row.internalAppFlagsName==null'>{{scope.row.peopleName=="内部人员"?"全部":"-"}}</span>
          <span v-else>{{scope.row.internalAppFlagsName}}</span>
        </template>  
      </el-table-column>
      <el-table-column prop="orgName" label="开放银行" header-align="center" align="center">
        <template slot-scope="scope" > 
          <span v-if='scope.row.orgName==""||scope.row.orgName==null'>{{scope.row.peopleName=="内部人员"?"全部":"-"}}</span>
          <span v-else>{{scope.row.orgName}}</span>
        </template>  
      </el-table-column>
      <el-table-column prop="prdIndexName" label="开放产品" header-align="center" align="center">
        <template slot-scope="scope" > 
          <span v-if='scope.row.prdIndexName==""||scope.row.prdIndexName==null'>{{scope.row.peopleName=="内部人员"?"全部":"-"}}</span>
...
<script>
import {WhitelistpageQuery,prdindexall ,org,WhitelistDelete} from '../../apis/index.js'
  ...
  importfile (obj) {
   ...
   reader.onload = function () {
    ...
    queryFun () {
      ...
      WhitelistpageQuery(params).then((res) =>{
        if (res.headerModel.code==0) {
          res.datatypeParam.retList.forEach((i) => {
            // 处理外部渠道字段
            if (i.appFlag==''||i.appFlag==null) {
              i.internalAppFlagsName=''
            }else if (i.appFlag.indexOf(',')<0) {
              switch(i.appFlag){
                case ('BC'):
      ...
      this.$confirm('此操作将删除该条管理员数据, 是否继续？', '提示', {type: 'warning'}).then(() => {
          ...
          datatypeParam:{
            ids: row.id
				...
				WhitelistDelete(params).then((res) => {
          console.log(res)
          if (res.headerModel.code==0) {
