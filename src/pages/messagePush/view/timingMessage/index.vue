...
<!-- 广告系列管理 -->
  <div class="timingMessage">
      ...
      <el-form-item label="目标渠道："  prop='appFlag' style='display:inline-block'>
        ...
        <el-form-item class="item"  prop='plateformType' style='display:inline-block;margin-left:10px'>
          <el-select style='width:40%' v-model="formSearch.plateformType" clearable placeholder="">
            <el-option
              v-for="item in plateformTypeOptions"
              :key="item.name"
              :label="item.name"
              :value="item.value">
            </el-option>
    ...
    <message-preview ref='messagePreview' :message='form' v-if='dialogFormVisible' :dialogFormVisible='dialogFormVisible' @sure='sure' @cancel='cancel'  title='发送预览' cancelTxt='' sureTxt='确 定'></message-preview>   
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    revokeActInfoByIdFun (row,index) {
      this.$confirm('是否撤回此条通知？', '提示', {type: 'warning'}).then(() => {
				let params= {
          id: row.id
        }
