...
<template>
  <section class=''>
    ...
    <el-row>
      ...
      v-modle的值：{{val3}}
      <br>
      <BcSelect v-model="val3" :isAutoWidth="true" itemNameKey="name" itemValueKey="value" @GetApiData="getOrgSelectData($event)" multiple :filterable="openRemote" :remote="openRemote"></BcSelect>
      <el-button @click="val3 = [{name:'步步为赢',value:'2000001599 '}]">回显已知数据</el-button>
      <el-button @click="val3 = [{name:'大象',value:'8'},{name:'吃',value:'9'},{name:'蚂蚁',value:'10'}]">回显未知数据</el-button>
      <el-button @click="openRemote = !openRemote">{{openRemote?"关闭":"开启"}}模糊查询</el-button>
    </el-row>
  </section>
</template>
