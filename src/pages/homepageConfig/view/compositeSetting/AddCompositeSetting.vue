        ...
        <el-form :size="size" ref="form" :rules="rules" :model="form" label-width="140px">
            ...
            <el-form-item label="跳转类型：" prop="contentType">
                <el-select clearable @change="changeContentType" v-model="form.contentType" placeholder="">
                    <el-option
                        :disabled="(form.configType == '1' && item.value != '3') || (form.configType == '2' && item.value == '3')"
            ...
            <el-form-item v-if="form.contentType != ''" label="内容" prop="content" style="position: relative">
                ...
                <el-select
                    ...
                    :filterable="form.contentType == 3 || form.contentType == 4 || form.contentType == 5"
                    remote
                    :remote-method="searchPro"
                    placeholder="">
                    <div v-if="form.contentType == 3 || form.contentType == 4 || form.contentType == 5">
                        <el-option
                            :class="form.contentType == 5 ? 'zixunClass' : ''"
            ...
            <el-form-item
                v-if="form.contentType != '3' && form.contentType != '' && (form.showType == '2' || form.showType == '3' || form.showType == '4')"
                class="imgDiv"
                label="图片："
                prop="imgUrlVl">
                <el-upload :model="form.imgUrl1" accept="image/*" class="upload-demo upload-demo1" action="customize" :http-request="uploadCheckImg1" :show-file-list="false">
            ...
            <el-form-item label="角标图片：" prop="lableUrl">
                <el-upload :model="form.lableUrl" accept="image/*" class="upload-demo upload-demo1" action="customize" :http-request="uploadChecklableUrl" :show-file-list="false">
            ...
            <el-form-item label="发布时间：" prop="time">
                <el-date-picker clearable v-model="time" type="daterange" value-format="yyyy-MM-dd HH:mm:ss" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期">
                </el-date-picker>
            </el-form-item>
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    contentType: [{ required: true, message: "请选择配置类型", trigger: "change" }],
                    content: [{ required: true, validator: contentVl, trigger: "change" }],
                    showType: [{ required: true, message: "请选择展示类型", trigger: "change" }],
                    title: [{ required: true, message: "请输入标题", trigger: "change" }],
                    time: [{ required: true, validator: timeVl, trigger: "change" }],
                    imgUrlVl: [{ required: true, validator: imgUrlVlVl, trigger: "change" }],
                    prdId: [{ required: true, message: "请输入产品id", trigger: "change" }],
                    seqOrder: [{ required: true, validator: seqOrderlVl, trigger: "change" }],
                    configType: [{ required: true, message: "请选择配置类型", trigger: "change" }],
                    isEndbled: [{ required: true, message: "请选择是否下架", trigger: "change" }],
                                ...
                                if (this.form.contentType == "5") {
                                    this.$nextTick(() => {
                                        for (var i = 0; i < document.getElementsByClassName("zixunClass").length; i++) {
                                            document
                                                .getElementsByClassName("zixunClass")
                                                [i].getElementsByTagName("span")[0]
                                                .setAttribute("title", document.getElementsByClassName("zixunClass")[i].getElementsByTagName("span")[0].innerHTML);
                                        }
                                    });
                                }
                            }
