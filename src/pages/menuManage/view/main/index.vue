...
<!-- 菜单管理 -->
  <section class='menuManage'>
    ...
    <!--表格树内容栏-->
    <el-table :height="tableHeight" size="mini" :data="tableTreeDdata" stripe  border style="width: 100%;"
    ...
    <el-dialog :title="!dataForm.id ? '新增' : '修改'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="submitForm()" 
        ...
        <el-form-item label="菜单图标" prop="attributes.icon">
          <el-row>
            <el-col :span="22">
              <el-input v-model="dataForm.attributes.icon" v-popover:iconListPopover :readonly="false" placeholder="菜单图标名称（如：el-icon-iconName）" class="icon-list__input"></el-input>
            </el-col>
            <el-col :span="2" class="icon-list__tips">
              <fa-icon-tooltip />
            </el-col>
...
export default {
  ...
  data () {
     return {
      ...
      dataForm: { //当前选中的列表数据，也是新增初始化和编辑回显的数据
        id: 0,
  ...
  methods: {
    ...
    findTreeData: function() {
        this.loading = true;
        let params = ''
        getMenuManages(params).then(data => {
