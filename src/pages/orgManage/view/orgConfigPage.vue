            ...
            <template slot-scope="scope">
              <el-form-item :prop="'dataForm.' + scope.$index + '.'+item.name" :rules="rules[item.name]">
                ...
                <div v-if='item.label == "机构类型" '>
                  <BcSelect v-model="scope.row.orgOptValue" :list="orgOption" :disabled="scope.row.isEdit" @change="orgSelectValue(scope.row)" placeholder="请选择机构类型"></BcSelect>
                </div>
                <div v-else-if='item.label == "选择机构"'>
                  <BcSelect v-model="scope.row.orgOptSelValue"
        ...
        //   console.log('单个删除',row)
          this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
                ...
                let params = {
                    ...
                    if(row.id){
                        this.$api.orgDelete(params).then(res => {
