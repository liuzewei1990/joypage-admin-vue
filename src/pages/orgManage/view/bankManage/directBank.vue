<template>
  ...
  <div class="prdSeries">
    <CommonTable :isFluid="false" @height="tableHeight=$event">
        ...
        <el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
          <el-form-item label="直销银行名称：" prop="id">
            <el-select v-model="topCheckData.id" placeholder="请选择" filterable clearable>
              <el-option v-for="item in allSelectData.directBankNameList" :key="item.id" :label="item.orgName" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="签约状态：" prop="isContract">
            <el-select v-model="topCheckData.isContract" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.isContractList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="银行所属地区：" prop="provinceAdcode">
            <el-select v-model="topCheckData.provinceAdcode" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.selProvinceList" :key="item.DIC_VALUE" :label="item.DIC_NAME" :value="item.DIC_VALUE">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="机构类型：" prop="orgType">
            <el-cascader v-model="orgType" :options="allSelectData.orgTypeList" :props="{ checkStrictly: true }" :show-all-levels="true" clearable @change="orgTypeListSelect"></el-cascader>
          </el-form-item>
        </el-form>
      </div>
      <!-- 操作按钮区 -->
      ...
      <!-- 表格内容栏 -->
      <el-table :height="tableHeight" size="mini" :data="directBankList" style="width: 100%" stripe border="true" @selection-change="selectionChange" v-loading="loading" element-loading-text="loading...">
      ...
      <!--分页栏-->
      <div slot="footer">
        <el-pagination layout="total, sizes, prev, pager, next, jumper" style="float:right;" :total="totalSize" @current-change="refreshPageRequest" @size-change="handleSizeChange" :current-page="pageRequest.pageNum" :page-sizes="[10, 20, 30, 40]" :page-size="pageRequest.pageSize">
        </el-pagination>
      </div>
    </CommonTable>
    <!-- 新增弹框 -->
      ...
      <el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size" label-position="left" class="addForm">
        ...
        <el-form-item label="一级机构类型" prop="levelOneOrgTypeId">
          <el-select v-model="dataForm.levelOneOrgTypeId" placeholder="请选择" clearable @change='levelOneOrgTypeIdSelect'>
            <el-option v-for="item in allSelectData.orgTypeList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="二级机构类型：" prop="levelTwoOrgTypeId">
          <el-select v-model="dataForm.levelTwoOrgTypeId" placeholder="请选择" clearable>
            <el-option v-for="item in allSelectData.levelTwoOrgTypeIdList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
        <template v-if="addType == '1'">
          <el-form-item label="银行名称：" prop="commercialId">
            ...
            <el-select v-model="dataForm.commercialId" placeholder="请选择" filterable clearable @change='bankNameListSelect'>
              <el-option v-for="item in allSelectData.bankNameList" :key="item.commercialId" :label="item.orgName" :value="item.commercialId">
              </el-option>
            </el-select>
          </el-form-item>
        </template>
        ...
        <el-form-item :label="addType == '1'?'直销银行名称首字母：':'机构名称首字母：'" prop="namePy">
          <el-input v-model="dataForm.namePy" :placeholder="addType == '1'?'请输入直销银行名称首字母':'请输入机构名称首字母'" @keyup.native="dataForm.namePy=dataForm.namePy.replace(/[^a-zA-Z]/g,'')"></el-input>
        </el-form-item>
        <!-- 新增非银机构 -->
        <template v-if="addType != '1'">
          <el-form-item label="机构归属地：" prop="orgLocation">
            <el-select v-model="dataForm.orgLocation" placeholder="请选择" clearable :style="dataForm.orgLocation == '1'? 'width:24%;padding-right:15px;float:left':''">
              <el-option v-for="item in allSelectData.orgLocationList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
            <!-- 当选择中国大陆时，联动选择省市 -->
        ...
        <el-form-item label="隶属于比财公司：" prop="contractCompany">
          <el-select v-model="dataForm.contractCompany" placeholder="请选择" clearable>
            <el-option v-for="item in allSelectData.contractCompanyList" :key="item.value" :label="item.contractCompanyName" :value="item.contractCompanyId">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <template v-if="addType == '1'">
          <el-form-item label="是否在“合作银行”展示：" prop="isAppVisible">
            <el-select v-model="dataForm.isAppVisible" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.isAppVisibleList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="是否在“行情”展示：" prop="isHangqing">
            <el-select v-model="dataForm.isHangqing" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.isAppVisibleListStr" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="行情机构分类：" prop="hangqingType">
            <el-select v-model="dataForm.hangqingType" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.hangqingTypeList" :key="item.dicValue" :label="item.dicName" :value="item.dicValue">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item class='fromItemBlue' label="是否在线开户：" prop="isOnline">
            <el-select v-model="dataForm.isOnline" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.isFlagListStr" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
        ...
        <el-form-item :label="addType == '1' ? ' 银行logo：': '机构logo：'" prop="logoUrl" :required="addType == 1 ?false:true">
          <el-upload :model='imageUrl + dataForm.logoUrl' class="upload-demo" ref="upload1" :accept="'.png'" action="#" :limit="limitNum" :on-change="fileChange" :on-exceed="exceedFile" :before-upload="beforeUploadFile" :http-request="uploadDirectBankPictureImg1" :show-file-list="false">
        ...
        <el-form-item class='fromItemBlue' :label="addType == '1' ? '文字logo：' : '机构文字logo：'" prop="logoWordUrl">
          <el-upload :model='imageUrl + dataForm.logoWordUrl' class="upload-demo" ref="upload2" :accept="'.png'" action="#" :limit="limitNum" :on-exceed="exceedFile" :http-request="uploadDirectBankPictureImg2" :show-file-list="false">
        ...
        <el-form-item :label="addType == '1' ? '银行主页背景图片：' : '机构主页背景图片：'" prop="logoBackgroundUrl" :required="addType == 1 ?false:true">
          <el-upload :model='imageUrl + dataForm.logoBackgroundUrl' class="upload-demo" ref="upload3" :accept="'.png'" action="#" :limit="limitNum" :on-exceed="exceedFile" :http-request="uploadDirectBankPictureImg3" :show-file-list="false">
        ...
        <el-form-item :label="addType == '1' ? '银行卡背景图片：':'账户背景图片：'" prop="bankcardUrl" :required="addType == 1 ?false:true">
          <el-upload :model='imageUrl + dataForm.bankcardUrl' class="upload-demo" ref="upload4" :accept="'.png'" action="#" :limit="limitNum" :on-exceed="exceedFile" :http-request="uploadDirectBankPictureImg4" :show-file-list="false">
        ...
        <template v-if="addType == '1'">
          <el-form-item class='fromItemBlue' label="带半圆银行背景图片：" prop="circleBackgroundUrl">
            <el-upload :model='imageUrl + dataForm.circleBackgroundUrl' class="upload-demo" ref="upload5" :accept="'.png'" action="#" :limit="limitNum" :on-exceed="exceedFile" :http-request="uploadDirectBankPictureImg5" :show-file-list="false">
          ...
          <el-form-item class='fromItemBlue' label="是否H5直连：" prop="openH5Status">
            <el-select v-model="dataForm.openH5Status" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.openApiOrH5StatusList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="允许多绑卡：" prop="isSupportBindMutiCards">
            <el-select v-model="dataForm.isSupportBindMutiCards" placeholder="请选择">
              <el-option v-for="item in allSelectData.isSupportBindMutiCardsList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item class='fromItemBlue' label="是否实名认证：" prop="isRzFlag">
            <el-select v-model="dataForm.isRzFlag" placeholder="请选择">
              <el-option v-for="item in allSelectData.isFlagList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="银行对接方式：" prop="bankDockType">
            <el-select v-model="dataForm.bankDockType" placeholder="请选择">
              <el-option v-for="item in allSelectData.bankDockTypeList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
        ...
        <el-form-item :label="addType == '1'? '介绍：' : '机构介绍：'" prop="instructionUrl">
          ...
          <el-card style="height: 480px;" v-loading="quillUpdateImg">
            <el-upload :model='dataForm.instructionUrl' class="avatar-uploader" :accept="acceptType" action="#" :http-request="quillEditorUploaderImg" :before-upload="qnUpload" :show-file-list="false">
            ...
            <quill-editor v-model="dataForm.instructionUrl" ref="myQuillEditor" style="height: 400px;" :options="editorOption" @change="onEditorChangeArea($event)">
            </quill-editor>
          </el-card>
        ...
        <el-form-item label="首页合作银行logo：" prop="homeLogoUrl">
          <el-upload :model='imageUrl + dataForm.homeLogoUrl' class="upload-demo" ref="upload6" :accept="'.png'" action="#" :limit="limitNum" :on-exceed="exceedFile" :http-request="uploadDirectBankPictureImg6" :show-file-list="false">
        ...
        <template v-if="addType == '1'">
          ...
          <el-form-item label="机构二级类型：" prop="secondLevelOrgType">
            ...
            <el-select v-model="dataForm.secondLevelOrgType" placeholder="请选择" clearable>
              <el-option v-for="item in allSelectData.bankSecondLevelOrgTypeList" :key="item.value" :label="item.name" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
...
import {
  queryOrgNameByBankType, queryDirectBankNameByBankType, queryOrgList, createOrg, updateOrg, deleteOrg,
  forUpload, selProvince, queryTypeByKey, configData, checkDuplicateSortNum, getOrgTypeList
} from "../../apis/index"
...
export default {
  ...
  data () {
    var isContractRule = (rule, value, callback) => { //签约状态
      if (this.addType == 1) {
        callback();
      } else {
        if (value === '') {
    ...
    var logoUrlRule = (rule, value, callback) => { //请上传机构logo
      if (this.addType == 1) {
        callback();
      } else {
        if (value === '') {
    ...
    var logoBackgroundUrlRule = (rule, value, callback) => { //机构主页背景图片
      if (this.addType == 1) {
        callback();
      } else {
        if (value === '') {
    ...
    var bankcardUrlRule = (rule, value, callback) => { //账户背景图片
      if (this.addType == 1) {
        callback();
      } else {
        if (value === '') {
    ...
    var namePyRule = (rule, value, callback) => { //机构名称首字母
      if (this.addType == 1) {
        callback();
      } else {
        if (value == '') {
  ...
  methods: {
    ...
    getOrgSelectData () {
      let params = ''
      //请求直销银行名称列表
      queryDirectBankNameByBankType(params).then(res => {
      ...
      selProvince(paramsCity).then(res => {
      ...
      // //请求银行所属类型
      queryTypeByKey(params).then(res => {
    ...
    submitFormSave: function (refdataForm) {
      ...
      if (this.dataForm.sortNum !== '') {
        ...
        //先查看排序值是否重复
        checkDuplicateSortNum(params).then((res) => {
          console.log("排序值接口", res);
          if (res.result == true) {
            if (res.data == true) { //排序值重复，则调新增或编辑的接口
              this.$confirm('排序值重复是否要进行插入操作', '提示', {}).then(() => { //确定
    ...
    onEditorChangeArea ({ quill, html, text }) {
      if (this.showType == 1) {//插入图片获取的富文本内容
        console.log('描述富文本html', html)
        this.dataForm.instructionUrl = html  //介绍
      } else if (this.showType == 2) { //插入视频获取的内容---功能暂未完成
    ...
    qnUpload (file) {
      ...
      if (this.showType === 1 && this.acceptType == 'image/*') {  // 如果是点击插入图片
      ...
      } else if (this.showType === 2 && this.acceptType == 'video/*') {  // 如果是点击插入视频
    ...
    quillEditorUploaderImg (content) {
      ...
      forUpload(formData).then((res) => {
        console.log(res)
        if (res.result) {
          if (this.showType === 1 && this.acceptType == 'image/*') { //获取图片地址
            //获取富文本组件实例
          ...
          } else if (this.showType === 2 && this.acceptType == 'video/*') { //获取视频地址---功能暂未完成
            // //获取富文本组件实例
            // let quill = this.$refs.myQuillEditor.quill
            // // 获取光标所在位置
            // let length = quill.getSelection().index;
