...
<!-- 银行动态管理 -->
  <div class="prdSeries">
		...
		<el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
			...
			<el-form-item label="动态状态" prop="dynamicStatus">
				<el-select v-model="topCheckData.dynamicStatus" placeholder="请选择" clearable>
					<el-option v-for="item in dynamicStatusList"
	...
	<!-- 表格内容栏 -->
	<el-table :height="tableHeight" size="mini" :data="bankDynamicList" style="width: 100%" stripe border="true"
		...
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="银行名称：" prop="orgId">
				...
				@change='orglistSelect'>
					<el-option v-for="item in orglist"
					:key="item.ID" :label="item.orgName" :value="item.commercialId">
					</el-option>
				</el-select>
				...
				<template>
					...
					<el-form-item class="details" label="详情：" prop="dynamicDetails" v-if='dataForm.dynamicType == 2?true:false'>
						...
						<el-card style="height: 400px;" v-loading="quillUpdateImg">
							...
							<quill-editor v-model="dataForm.dynamicDetails" ref="myQuillEditor" style="height: 300px;" 
			...
			<el-form-item label="发布时间：" prop="startDate">
				<el-date-picker v-model="dataForm.startDate" type="date" placeholder="开始时间"
				format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" style="width: 28%;"></el-date-picker>
				<el-date-picker v-model="dataForm.endDate" type="date" placeholder="结束时间"
				format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" style="width: 28%;margin-left:30px"></el-date-picker>
			</el-form-item>
			<el-form-item label="紧急下架：" prop="emergencyOffline">
				<el-select v-model="dataForm.emergencyOffline" placeholder="请选择">
					<el-option v-for="item in isFlagList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
...
export default {
	...
	methods: {
		...
		getbankDynamicList: function(type) {
			...
			queryBankDynamicsList(params).then(res => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						createBankDynamics(params).then((res) => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updBankDynamics(params).then((res) => {
							if(res.result == true){
								this.saveLoading = false
								this.$message({ message: res.description, type: 'success' })
								this.dialogVisible = false
		...
		handleDelete (index, row) {
			...
			this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
				...
				delBankDynamics(params).then(res => {
