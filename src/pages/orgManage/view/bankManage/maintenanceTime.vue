...
<!-- 银行产品维护时间管理 -->
  <div class="prdSeries">
	<CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<div slot="search">
			...
			<el-form-item label="银行名称：" prop="orgName">
					<el-select v-model="topCheckData.orgName" placeholder="请选择" filterable clearable>
						<el-option v-for="item in allSelectData.orgNameList"
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="maintainTimeList" style="width: 100%" stripe border="true"
	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" width="70%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="银行名称：" prop="orgId">
				<!-- filterable开启可搜索模式  -->
				<el-select v-model="dataForm.orgId" placeholder="请选择" filterable clearable
				@change='orgNameListSelect'>
					<el-option v-for="item in allSelectData.orgNameList"
            ...
            <el-form-item label="操作类型：" prop="maintainType">
				<el-select v-model="dataForm.maintainType" placeholder="请选择" clearable
				@change='maintainTypeListSelect'>
					<el-option v-for="item in allSelectData.maintainTypeList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
            ...
            <template v-if="dataForm.remindDate == '0'">
               <el-form-item label="开始时间：" prop="startDate">
                    <el-date-picker v-model="dataForm.startDate" type="datetime" placeholder="开始日期时间" :picker-options="pickerOptionsStart"
                    format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" style="width: 50%;"></el-date-picker>
                </el-form-item>
                <el-form-item label="结束时间：" prop="endDate">
                    <el-date-picker v-model="dataForm.endDate" type="datetime" placeholder="结束日期时间" :picker-options="pickerOptionsEnd"
                    format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss" style="width: 50%;"></el-date-picker>
                </el-form-item>
            </template>
            <!-- 每周几，则分开选日期和时间 -->
            <template v-else>
                <el-form-item label="开始时间：" prop="startDate">
                    <el-date-picker v-model="dataForm.startDate" type="date" placeholder="开始时间" :picker-options="pickerOptionsStart"
                    format="yyyy-MM-dd" value-format="yyyy-MM-dd HH:mm:ss" style="width: 50%;"></el-date-picker>
                </el-form-item>
                <el-form-item label="结束时间：" prop="endDate">
                    <el-date-picker v-model="dataForm.endDate" type="date" placeholder="结束时间" :picker-options="pickerOptionsEnd"
                    format="yyyy-MM-dd" value-format="yyyy-MM-dd HH:mm:ss" style="width: 50%;" ></el-date-picker>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		getMaintainTimeList: function(type) {
			...
			queryBankProductsWeiList(params).then(res => {
		...
		getQueryPrdByOrgId(val){
			...
			queryPrdByOrgId(paramsPrd).then(res => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						createBankProductsWei(params).then((res) => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updBankProductsWei(params).then((res) => {
		...
		handleDelete (index, row) {
			...
			this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
				...
				delBankProductsWei(params).then(res => {
