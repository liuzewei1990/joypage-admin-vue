...
<!-- 绑定卡管理 -->
  <div class="prdSeries">
	<CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<div slot="search">
			...
			<el-form-item label="银行名称：" prop="elecName">
					<el-select v-model="topCheckData.elecName" placeholder="请选择" filterable clearable>
						<el-option v-for="item in allSelectData.elecNameList"
				...
				<el-form-item label="支持操作类型：" prop="maintainType">
					<el-select v-model="topCheckData.maintainType" placeholder="请选择" clearable>
						<el-option v-for="item in allSelectData.maintainTypeList"
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="bindingCardList" style="width: 100%" stripe border="true"
			...
			<el-table-column prop="maintainType" label="支持操作类型" header-align="center" align="center">
				<template slot-scope="scope">
					<p :size="size" v-for="(item,index) in  allSelectData.maintainTypeList" :key="index" 
						>{{scope.row.maintainType == item.value?item.name:''}}</p>
	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" width="70%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="电子账户名称：" prop="elecId">
				<!-- filterable开启可搜索模式  -->
				<el-select v-model="dataForm.elecId" placeholder="请选择" filterable clearable
				@change='elecNameListSelect'>
					<el-option v-for="item in allSelectData.elecNameList"
					:key="item.commercialId" :label="item.orgName" :value="item.commercialId">
					</el-option>
				</el-select>
			</el-form-item>
            <el-form-item label="银行卡名称：" prop="cardId">
				<el-select v-model="dataForm.cardId" placeholder="请选择" clearable
				@change='bankCardNameListSelect'>
					<el-option v-for="item in allSelectData.bankCardNameList"
					:key="item.id" :label="item.orgName" :value="item.id">
					</el-option>
				</el-select>
			</el-form-item>
            <el-form-item label="是否支持大额转入：" prop="isLargeAmount">
				<el-select v-model="dataForm.isLargeAmount" placeholder="请选择" clearable
				@change="isLargeAmountSelect">
					<el-option v-for="item in allSelectData.isLargeAmountList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
...
import {queryDirectBankNameByBankType,queryOrgNameByBankType,queryBankCard,queryElecAccountList,createElecAccount,updateElecAccount,deleteElecAccount,deleteElecAccountList} from "../../apis/index"
...
export default {
	...
	data() {
		return {
			...
			formRules: { //弹框校验规则
				elecId:[ 
					{ required: true, message: '请选择电子账户名称', trigger: 'blur' },
				],
				bankCardId: [ 
					{ required: true, message: '请选择银行卡名称', trigger: 'blur' },
				],
				isLargeAmount: [ 
					{ required: true, message: '请选择是否支持大额转入', trigger: 'blur' },
                ],
                maintainType: [ 
					{ required: true, message: '请选择支持操作类型', trigger: 'blur' },
                ],
                singleRechargeLimit: [ 
					{ required: true, message: '请输入充值单笔限额', trigger: 'blur' },
                ],
                accLimitRechargeDay: [ 
					{ required: true, message: '请输入充值日累计限额', trigger: 'blur' },
                ],
                rechargeMonthlyPaymentLimit: [ 
					{ required: true, message: '请输入充值月支付限额', trigger: 'blur' },
                ],
                singleLimitWithdrawal: [ 
					{ required: true, message: '请输入提现单笔限额', trigger: 'blur' },
                ],
                accLimitWithdrawalDay: [ 
					{ required: true, message: '请输入提现日累计限额', trigger: 'blur' },
                ],
                withdrawalMonthlyPaymentLimit: [ 
					{ required: true, message: '请输入提现月支付限额', trigger: 'blur' },
                ],
			},  
			//删除
			selections: [], //删除选中项
	...
	methods: {
		...
		getBndingCardList: function(type) {
			...
			queryElecAccountList(params).then(res => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						if(this.dataForm.isLargeAmount == '3'){ //仅支持
						...
						createElecAccount(params).then((res) => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updateElecAccount(params).then((res) => {
		...
		handleDelete (index, row) {
			...
			this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
				...
				deleteElecAccount(params).then(res => {
		...
		handleBatchDelete: function () {
			...
			this.$confirm('确认删除选中记录吗？', '提示', {type: 'warning'}).then(() => {
				...
				deleteElecAccountList(params).then(res => {
					console.log("删除接口",res);
					if(res.result == true){
						this.$message({message: '删除成功', type: 'success'})
						this.getBndingCardList() //刷新当前页
