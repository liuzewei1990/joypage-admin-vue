...
<!-- 证券公司管理 -->
  <div class="prdSeries">
	<CommonTable :isFluid="false" @height="tableHeight=$event">
			...
			<el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
				<el-form-item label="证券公司名称：" prop="name">
					<el-select v-model="topCheckData.name" placeholder="请选择" filterable clearable>
						<el-option v-for="item in securitiesCompanyNameList"
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="securitiesCompanyList" style="width: 100%" stripe border="true"
...
export default {
	...
	data() {
		return {
			...
			formRules: { //弹框校验规则
				...
				code:[
					{ required: true, message: '请输入证券公司编码', trigger: 'blur' },
				],
				logoUrl:[
	...
	methods: {
		...
		getnameByBankType(){
			let params = ''
			querySecuritiesNames(params).then(res => {
		...
		getSecuritiesCompanyList: function(type) {
			...
			querySecuritiesList(params).then(res => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						createSecurities(params).then((res) => {
		...
		handleDelete (index, row) {
			...
			this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
				...
				deleteSecurities(params).then(res => {
