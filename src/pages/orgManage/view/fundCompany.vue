  ...
  <div class="prdSeries">
	<CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<div slot="search">
			<el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
				<el-form-item label="基金公司名称：" prop="fundCompanyName">
					<el-select v-model="topCheckData.fundCompanyName" placeholder="请选择" filterable clearable>
						<el-option v-for="item in allSelectData.fundCompanyNameList"
						:key="item.id" :label="item.fundCompanyName" :value="item.fundCompanyName">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="状态：" prop="fundStatus">
					<el-select v-model="topCheckData.fundStatus" placeholder="请选择" filterable clearable>
						<el-option v-for="item in allSelectData.fundStatusList"
		...
		<div slot="action">
			...
			<el-button icon="el-icon-search" style="float:right;" :size="size" type="primary" @click="getfundCompanyList('chaxun')">查询</el-button>
		</div>
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="fundCompanyList" style="width: 100%" stripe border="true"
			...
			<el-table-column prop="id"  label="基金公司ID" header-align="center" align="center"></el-table-column>
			<el-table-column prop="fundCompanyName" label="基金公司名称" header-align="center" align="center"></el-table-column>
			<el-table-column prop="fundCount" label="旗下基金数" header-align="center" align="center"></el-table-column>
			<el-table-column prop="fundAssetsSize" label="资产规模（亿元）" header-align="center" align="center"></el-table-column>
			<el-table-column prop="registDate" label="注册日期" header-align="center" align="center"></el-table-column>
			<el-table-column prop="fundStatus" label="状态" width='80px'  header-align="center" align="center">
				<template slot-scope="scope">
					<p v-if="scope.row.fundStatus=='1'" :size="size">正常</p>
					<p v-if="scope.row.fundStatus=='0'" :size="size">弃用</p>
				</template>
			</el-table-column>
	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" width="60%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="基金公司名称：" prop="fundCompanyName">
				<el-input v-model="dataForm.fundCompanyName" placeholder="请输入基金公司名称"></el-input>
			</el-form-item>
            ...
            <el-form-item label="旗下基金数：" prop="fundCount">
				<el-input v-model="dataForm.fundCount" placeholder="请输入旗下基金数"></el-input>
			</el-form-item>
			<el-form-item label="资产规模（亿元）：" prop="fundAssetsSize">
				<el-input v-model="dataForm.fundAssetsSize" placeholder="请输入资产规模（亿元）"></el-input>
			</el-form-item>
			<el-form-item label="注册时间：" prop="registDate">
				 <el-date-picker v-model="dataForm.registDate" type="date" placeholder="注册时间" 
                    format="yyyy-MM-dd" value-format="yyyy-MM-dd HH:mm:ss" style="width: 50%;"></el-date-picker>
			</el-form-item>
            <el-form-item label="状态：" prop="fundStatus">
				<el-select v-model="dataForm.fundStatus" placeholder="请选择" clearable>
					<el-option v-for="item in allSelectData.fundStatusList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
...
<script>
// 接口函数依次：查询基金公司名称，查询列表，新增，修改，删除
import {queryFundCompanyNames,queryFundCompanyList,createFundCompany,updFundCompany,deleteFundCompany,forUpload} from "../apis/index"
...
export default {
	...
	data() {
		return {
			...
			totalSize: 0, //总数
			pageRequest: { //请求基金公司列表参数
                pageNum: 0, //当前页数
                pageSize: 10  //条数
			},
			loading:false, //请求基金公司时loading
			fundCompanyList: [], //请求回来的基金公司列表数据list
			allSelectData:{
				fundCompanyNameList:[],//基金公司名称
				fundStatusList:[
					{
						'name':'正常',
						'value':'1'
			...
			dataForm: {  //新增弹框数据
				...
				fundCompanyName:'', //基金公司名称
				fundCount:'', //旗下基金数量
				fundAssetsSize:'', //基金公司规模（单位亿元）0.00
				registDate:'', //注册日期
				fundStatus:'', //状态，正常：1，删除：0
				updId:'', //修改人
				logoUrl: '', //logo图片地址
			},
			formRules: { //弹框校验规则
				...
				fundCount:[
					{ required: true, message: '请输入旗下基金数量', trigger: 'blur' },
				],
				logoUrl:[
  	...
  	mounted() {
		...
		this.getfundCompanyNameByBankType() //下拉框基金公司名称数据
		this.getfundCompanyList() //基金公司管理列表
	},
	methods: {
		//请求基金公司名称列表
		getfundCompanyNameByBankType(){
			let params = ''
			queryFundCompanyNames(params).then(res => {
				console.log("获取基金公司名称",res)
				if(res.result == true){
					//机构列表
					this.allSelectData.fundCompanyNameList = res.data
				}else{
					console.log('获取基金公司名称失败')
					this.loading = false;
					this.$message({ message: res.description, type: 'error' })
				}
			}).catch((err) => {
				console.log("基金公司名称请求失败err",err);
				this.loading = false;
				this.$message({ message: '基金公司名称请求失败', type: 'error' })
			})
		},
		// 获取基金公司列表数据
		getfundCompanyList: function(type) {
			this.loading = true;
			if(type == 'chaxun'){
				console.log('chauxn')
			...
			let params = {
				...
				fundCompanyName: this.topCheckData.fundCompanyName, //基金公司名称
				fundStatus: this.topCheckData.fundStatus,//状态
			}
			console.log('基金公司管理列表数据入参',params)
			queryFundCompanyList(params).then(res => {
				console.log("获取基金公司列表数据",res)
				if(res.result == true){
					this.loading = false;
					this.fundCompanyList = res.data.data
				...
				console.log("基金公司管理数据入参err",err);
				this.loading = false;
				this.$message({ message: '请求基金公司列表数据失败', type: 'error' })
			})
		},
		// 点击页数--分页刷新 
		refreshPageRequest: function (val) {
			...
			this.getfundCompanyList() //调基金公司列表接口
		},
		// 点击条数--分页刷新
		handleSizeChange(val){
			console.log(`当前条数: ${val}`);
			this.pageRequest.pageSize = val
			this.getfundCompanyList() //调产品系列列表接口
		},
		//重置
		resetForm(topData){
			console.log('重置')
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updFundCompany(params).then((res) => {
		...
		handleDelete (index, row) {
			...
			if(row.fundStatus == '0'){
				this.$message({type: 'info',message: '改条基金公司数据，已为弃用数据，无需删除，如需使用，可在修改中更改状态！'});
			}else{
				this.$confirm('确定要删除此条内容？', '提示', {type: 'warning'}).then(() => {
					...
					deleteFundCompany(params).then(res => {
						console.log("删除接口",res);
						if(res.result == true){
							this.loading = false
							this.$message({message: res.description, type: 'success'})
							this.getfundCompanyList() //刷新当前页
