  ...
  <section class="" data="数据注水">
    ...
    <el-row class="row-col-content" v-if="!isDetail" v-for="(formItem, index) in formList" :key="index">
      ...
      <el-col :style="{ width: col.s1 }">
        <!-- 选择机构，查询产品，必填项 -->
        <el-form-item class="spacing" :prop="`prdList[${index}].org`" :rules="{ required: true, message: '请选择机构', trigger: 'change', validator: $validator.bcSelect() }" :show-message="true">
          <BcSelect v-model="formItem.org" :key="formItem.prdType.id" @GetApiData="SetOrgSelectData($event, formItem)" filterable @change="formItem._orgIdOnChange(formItem)" placeholder="请选择机构"></BcSelect>
        </el-form-item>
      </el-col>
      ...
      <el-col :style="{ width: col.s2 }">
        <el-form-item class="spacing" :prop="`prdList[${index}].prd`" :rules="{ required: true, message: '请选择产品', trigger: 'change' }" :show-message="true">
          <BcSelect v-model="formItem.prd" :key="formItem.org.id" :disabled="!formItem.org.id" @GetApiData="SetPrdSelectData($event, formItem)" filterable placeholder="请选择产品"></BcSelect>
        </el-form-item>
      </el-col>
      ...
      <!-- 图片上传 -->
      <Upload title="上传机构图片" v-model="formItem.upload" :limit="1" @onSuccess="$emit('validate', `prdList[${index}].upload`)" @handleExceed="$msg.fail('机构图片只能上传一张，请先删除在上传')" upTip="只能上传jpg/png文件，且不超过500kb，图片尺寸为宽600 x 高152" :visible.sync="formItem._uploadVisible"></Upload>
    </el-row>
