...
<!-- 权限管理 -->
  <div class="rightsManage">
	<CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="pageRoletList" style="width: 100%" stripe border="true"
	...
	<el-dialog title="新增角色" width="40%" :visible.sync="addFormVisible">
		<el-form :model="addForm" label-width="80px" :rules="formRules" ref="addForm" :size="size"
	...
	<el-dialog :title="'编辑角色：'+ currentRoleName" width="40%" :visible.sync="editFormVisible">
		<el-form :model="editForm" label-width="80px" :rules="formRules" ref="editForm" :size="size"
			label-position="right">
	...
	<el-dialog :title="'授权角色：' + currentRoleName" width="40%" :visible.sync="roleListVisible" >
		...
		<el-tree :data="roleTreeData" :props="defaultProps" node-key="id" ref="elTree"
		:default-checked-keys="defaultCheckedKeys"  @check-change="checkChange"
		:load="loadNode"  show-checkbox  empty-text=""
		v-loading="roleListLoading" element-loading-text="拼命加载中..." >
		</el-tree>
		<div slot="footer" class="dialog-footer">
			<el-button :size="size" @click.native="roleListVisible = false">取消</el-button>
			<el-button :size="size" type="primary" :disabled="roleSaveBtnDisabled" 
			@click.native="submitFormRoleSaveBtn()" :loading="saveLoading">提交</el-button>
		</div>
	</el-dialog>
  </div>
</template>
...
export default {
	...
	methods: {
		...
		findRolePage: function() {
			...
			getRoleManages(params).then(data => {
			...
			this.$refs.addForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						getRoleAddSave(addFormData).then((res) => {
			...
			this.$refs.editForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						getRoleEditSave(editFormData).then((res) => {
							this.saveLoading = false
							this.$message({ message: '操作成功', type: 'success' })
		...
		submitFormRoleSaveBtn () {
			console.log('授权提交');
			this.$confirm('确认提交选中记录吗？', '提示', {}).then(() => {
				this.saveLoading = true
				...
				//调授权保存的接口
				getRoleGrantSave(params).then((res) => {
