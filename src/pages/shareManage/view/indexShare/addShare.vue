...
<!-- 新增/修改分享 -->
  <div class='addShare'>
    <el-form  :size='size' ref="form" :rules="rules" :model="form" label-width="160px">
      ...
      <el-form-item label="分享入口图片："  prop="entranceImgUrl">
        <el-upload
          :model='form.entranceImgUrl'
          class="upload-demo"
          action="customize"
  ...
  <!-- 分享方式  为小程序 -->
      <div v-if='form.shareType=="2"'>
        ...
        <el-form-item label="小程序分享内容："  prop="appletShareContent">
          ...
          type="textarea"
          placeholder=""
          v-model="form.appletShareContent"
          ></el-input>
        </el-form-item>
        <el-form-item  label="小程序图片："  prop="appletImgUrl">
          <el-upload
            :model='form.appletImgUrl'
            class="upload-demo upload-demo1"
            action="customize"
        ...
        <div class='second'>
          ...
          <el-form-item v-if='showShareMathods' label="备用链接地址："  prop="appletSpareLinkAddress">
            ...
            type="text"
            placeholder=""
            v-model="form.appletSpareLinkAddress"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item v-if='showShareMathods' label="备用链接分享内容："  prop="appletSpareContent">
            ...
            type="text"
            placeholder=""
            v-model="form.appletSpareContent"
            ></el-input>
          </el-form-item>
        </div>
      </div>
...
<!--  选择短信邀请下方功能为-->
      <div v-if='form.shareDestination==="5"' class='second'>
        ...
        <el-form-item label="邀请间隔天数："  prop="inviteIntervalDays">
          ...
          type="text"
          placeholder=""
          v-model="form.inviteIntervalDays"
        ></el-input>
        </el-form-item>
      </div>
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        entranceImgUrl:[ 
          { required: true, message: '请上传分享入口图片', trigger: 'blur' }
        ],
        shareDestination:[ 
          { required: true, message: '请选择分享至', trigger: 'change' }
        ],
        activityId:[ 
        ...
        inviteIntervalDays:[ 
          { required: true, message: '请输入邀请间隔天数', trigger: 'blur' }
        ],
        messageArticle:[ 
          { required: true, message: '请输入短信发送文案', trigger: 'blur' }
        ],
        shareImgUrl:[ 
          { required: true, message: '请上传分享图片', trigger: 'blur' }
        ],
        appletShareTheme:[ 
          { required: true, message: '请输入小程序分享主题', trigger: 'blur' }
        ],
        appletShareContent:[ 
          { required: true, message: '请输入小程序分享内容', trigger: 'blur' }
        ],
        appletSpareShareType:[ 
          { required: true, message: '请选择备用分享方式', trigger: 'blur' }
        ],
        appletImgUrl:[ 
          { required: true, message: '请上传小程序图片', trigger: 'blur' }
        ],
        appletSpareLinkAddress:[ 
          { required: true, message: '请输入备用链接地址', trigger: 'blur' }
        ],
        appletSpareTheme:[ 
          { required: true, message: '请输入备用链接分享主题', trigger: 'blur' }
        ],
        appletSpareContent:[ 
          { required: true, message: '请输入备用链接分享内容', trigger: 'blur' }
        ],
        theme:[ 
  ...
  created () {
    if (this.$route.query.id) {
      ...
      forShareDetail(params,this.$route.query.id).then((res) => {
        console.log('res',res)
        this.form=res
        this.imageUrl=`${this.$Config.aliyuncHost}${res.entranceImgUrl}`
        this.imageUrlMin=`${this.$Config.aliyuncHost}${res.appletImgUrl}`
        this.form.appletSpareShareType=res.appletSpareShareType.toString()
        this.form.shareType=res.shareType.toString()
        this.form.shareDestination=res.shareDestination.toString()
  ...
  methods: {
    ...
    uploadCheckImg(params) {
      ...
      forUpload(formData).then((res) => {
        if (res.result) {
          this.form.entranceImgUrl=res.data[0]
          ...
          if (this.type=='add') {
            ...
            addShare(params).then((res) => {
              if (res.result) {
                ...
                this.$router.push('/shareList/indexShareList')
              } else {
                this.saveLoading=false
                this.$message.error(res.description);
              }
