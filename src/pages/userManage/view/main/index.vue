...
<!-- 用户管理 -->
  <div class="userManage">
	  <CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="pageResultList" style="width: 100%" stripe border="true"
	...
	<el-dialog title="新增用户" width="40%" :visible.sync="addFormVisible">
		<el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm" :size="size"
			...
			<el-form-item label="角色" prop="roleName">
				<el-select v-model="addForm.roleId" placeholder="请选择" style="width: 100%;">
					<el-option v-for="item in roleTreeOptions" :key="item.ID"
	...
	<el-dialog title="编辑" width="40%" :visible.sync="editFormVisible">
		<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm" :size="size"
			label-position="right">
			<el-form-item label="登录账号" prop="account">
...
export default {
	...
	methods: {
		...
		findUserPage: function() {
			...
			getUserManages(params).then(data => {
			...
			this.$refs.addForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						getUserAddSave(addFormData).then((res) => {
			...
			this.$refs.editForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						getUserEditSave(editFormData).then((res) => {
							this.saveLoading = false
							this.$message({ message: '操作成功', type: 'success' })
