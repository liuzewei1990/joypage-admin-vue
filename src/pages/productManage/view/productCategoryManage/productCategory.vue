...
<!-- 产品管理-产品类别管理 -->
  <div class="productCategory">
    ...
    <el-table size="mini" :data="prdTypeList" style="width: 100%" stripe border="true"
		@selection-change="selectionChange" v-loading="loading" element-loading-text="loading..."
		default-expand-all row-key="id" :tree-props="{children: 'subTypeList', hasChildren: 'hasChildren'}" >
	...
	<el-dialog title="修改" width="50%" :visible.sync="editFormVisible" :before-close="closeDialog">
		<el-form :model="editForm" label-width="80px" :rules="formRules" ref="editForm" :size="size"
			...
			<el-form-item label="产品大类" prop="parentId">
				...
				@change='getPrdTypeSelect'>
					<el-option v-for="item in prdTypeParentList"
					:key="item.id" :label="item.typeName" :value="item.id">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="产品类型" prop="id">
				<el-select v-model="editForm.id" placeholder="请选择" clearable
				@change='getPrdSubTypeSelect'>
					<el-option v-for="item in prdTypeSonList"
					:key="item.id" :label="item.typeName" :value="item.id">
					</el-option>
				</el-select>
			</el-form-item>
               ...
               <el-table-column prop="typeName" label="产品大类名称" align="center">
                   <template slot-scope="scope">
                        <el-form-item :prop="`${scope.$index}.typeName`"  :rules="formRules.typeName">
                            ...
                            <el-select v-model="scope.row.typeName" :disabled="scope.row.isEdit" placeholder="请选择" clearable
                            @change='getPrdTypeSelect'>
                                <el-option v-for="item in prdTypeParentList"
                                :key="item.id" :label="item.typeName" :value="item.id">
                                </el-option>
  ...
  mounted() {
	...
	methods: {
		...
		getPrdTypeList: function() {
			this.loading = true;
			let params = '';
			forPrdTypeList(params).then(res => {
				console.log("获取产品类别列表",res)
		...
		getPrdCategoryPage(){
			let params = ''
			forPrdCategoryPage(params).then(res => {
			...
			this.$refs.editForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updatePrdType(this.editForm).then((res) => {
							console.log('修改保存请求成功',res)
							this.saveLoading = false
							this.$message({ message: '操作成功', type: 'success' })
