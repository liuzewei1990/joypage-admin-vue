...
<!-- 平台权益配置管理 -->
  <div class="platformRights">
	  <CommonTable :isFluid="false" @height="tableHeight=$event">
			...
			<el-form :inline="true" :model="topCheckData" :size="size" ref="topData">
				<el-form-item label="平台权益付息规则：" prop="payRule">
					<el-select v-model="topCheckData.payRule" placeholder="请选择平台权益付息规则" clearable>
						<el-option v-for="item in allSelectData.payRuleList"
				...
				<el-form-item label="补贴渠道：" prop="appFlag">
					<el-select v-model="topCheckData.appFlag" placeholder="请选择补贴渠道" clearable>
						...
						:key="item.DIC_VALUE" :label="item.DIC_NAME" :value="item.DIC_VALUE">
						</el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="补贴状态：" prop="subsidyStatus">
					<el-select v-model="topCheckData.subsidyStatus" placeholder="请选择补贴状态" clearable>
						<el-option v-for="item in allSelectData.statusList"
						:key="item.value" :label="item.label" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="银行名称：" prop="orgName">
					<el-select v-model="topCheckData.orgName" placeholder="请选择银行名称" filterable clearable>
						...
						:key="item.id" :label="item.orgName" :value="item.orgName">
						</el-option>
					</el-select>
				</el-form-item>
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="platformRightsList" style="width: 100%" stripe border="true"
	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" width="60%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="权益补贴对应产品：" prop="prdIndexId">
				<el-select v-model="dataForm.prdIndexId" placeholder="请选择权益补贴对应产品" filterable clearable
            ...
            <el-form-item label="利率期限（用于公式）：" prop="timeLimit">
				<el-select v-model="dataForm.timeLimit" placeholder="请输入利率期限（用于公式）" clearable>
					<el-option v-for="item in allSelectData.timeLimitList"
					:key="item.value" :label="item.label" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
			<el-form-item label="平台权益付息规则：" prop="payRule">
				<el-select v-model="dataForm.payRule" placeholder="请选择平台权益付息规则" clearable
				@change='payRuleListSelect'>
					<el-option v-for="item in allSelectData.payRuleList"
					:key="item.value" :label="item.label" :value="item.value">
					</el-option>
				</el-select>
			...
			<el-form-item label="最低金额：" prop="minAmountType" v-if="dataForm.payRule != '6'">
				<el-select v-model="dataForm.minAmountType" placeholder="请选择最低金额类型" clearable style="width:35%">
					<el-option v-for="item in allSelectData.minAmountTypeList"
					:key="item.value" :label="item.label" :value="item.value">
					</el-option>
				</el-select>
			...
			<!-- 1186渠道名称不上二期再上 针对返回值-->
			 <el-form-item label="渠道名称：" prop="yiyeChannelId" >
				<el-select v-model="dataForm.yiyeChannelId" placeholder="请选择渠道名称" clearable>
					<el-option v-for="item in allSelectData.yiyeChannelIdList"
					:key="item.channelId" :label="item.channelName" :value="item.channelId">
					</el-option>
				</el-select>
			</el-form-item>
			...
			<el-form-item label="用户分群：" prop="personGroupList">
				<el-select v-model="dataForm.personGroupList" placeholder="请选择用户分群" clearable multiple>
					<el-option v-for="item in allSelectData.personGroupList"
					:key="item.DIC_VALUE" :label="item.DIC_NAME" :value="item.DIC_VALUE">
					</el-option>
				</el-select>
			</el-form-item>
...
export default {
	...
	data() {
		return {
			...
			formRules: { //弹框校验规则
				prdIndexId: [
					{ required: true, message: '请输入权益补贴对应产品', trigger: 'blur' },
				],
				floatRate: [
					{ required: true, message: '请输入补贴利率', trigger: 'blur' },
				],
				timeLimit:[
					{ required: true, message: '请选择利率期限', trigger: 'change' }
				],
				payRule:[
					{ required: true, message: '请选择平台权益付息规则', trigger: 'change' }
				],
				payInterestPero:[
					{ required: true, message: '请选择付息周期', trigger: 'change' }
				],
				minDays:[
					{ required: true, message: '请输入最低持仓时间要求（天）', trigger: 'blur' }
				],
				systemType:[
					{ required: true, message: '请选择补贴展示平台', trigger: 'change' }
				],
				personGroup:[
				...
				minAmountType:[
					{ required: true, message: '请选择最低金额', trigger: 'change' }
				],
				minAmount:[
  ...
  mounted() {
	...
	methods: {
		...
		getPlatformRightsList: function(type) {
			...
			//调用平台权益列表接口
			queryPrdFloatRateList(params).then(res => {
				// let res = {
					// 		data:{
					// 			recordsTotal:'100',
					// 			data:[{
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						createPrdFloatRate(params).then((res) => {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						//请求编辑接口
						updPrdFloatRate(params).then((res) => {
