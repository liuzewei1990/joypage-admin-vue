	...
	<CommonTable :isFluid="false" @height="tableHeight=$event">
			...
			<el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
				...
				<el-form-item label="基金类型：" prop="prdTypeId">
					...
					@change='searchOrglistSelect'>
						<el-option v-for="item in allSelectData.fundTypeList"
						:key="item.value" :label="item.name" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
		...
		<div slot="action">
			...
			<el-button class="importBtn" style="float:left;" :size="size" icon="el-icon-sort-down" type="primary" 
				@click="handleImport(1)">货币基金导入</el-button>
			...
			<el-button class="importBtn" style="float:left;" :size="size" icon="el-icon-sort-down" type="primary" 
				@click="handleImport(3)">纯债基金导入</el-button>
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="financialFundList" style="width: 100%" stripe border="true"
	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" width="80%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="基金类型：" prop="prdIndexDTO.prdTypeId">
				...
				@change='fundTypeListSelect' :disabled="dataForm.id">
					<el-option v-for="item in allSelectData.fundTypeList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
            ...
            <el-form-item label="所属基金公司名称：" prop="prdIndexDTO.fundId">
				<el-select v-model="dataForm.prdIndexDTO.fundId" placeholder="请选择" clearable
			...
			<el-form-item label="理财系列：" prop="prdIndexDTO.prdSeriesId">
				<el-select v-model="dataForm.prdIndexDTO.prdSeriesId" placeholder="请选择类型和机构名称后，再选择系列" clearable
				@change='depositSeriesListSelect'>
					<el-option v-for="item in allSelectData.prdSeriesList"
					:key="item.value" :label="item.name" :value="item.id">
					</el-option>
			...
			<template v-if="dataForm.prdIndexDTO.prdTypeId == 1" prop="prdIndexDTO">
				...
				:rules="[{ required: true, message: '请输入七日年化收益率', trigger: 'blur' }]">
					<el-date-picker v-model="dataForm.prdIndexDTO.rateTime" type="date" format="yyyy-MM-dd" 
					value-format="yyyy-MM-dd" placeholder="请选择时间" style="width:30%"></el-date-picker>
				...
				<el-form-item label="万份年化收益（%）：" 
				:rules="[{ required: true, message: '请输入万份年化收益', trigger: 'blur' }]">
            ...
            <template v-if="dataForm.prdIndexDTO.prdTypeId == 3">
                <el-form-item label="交易配置：" prop="transactionStatus">
                    <el-select v-model="dataForm.transactionStatus" placeholder="请选择" clearable>
                        <el-option v-for="item in allSelectData.transactionStatusList"
                        :key="item.value" :label="item.name" :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item>
				<!--要是显示校验的提示 需要加 prop="prdIndexDTO.unitNet" -->
                <el-form-item label="单位净值：" prop="prdIndexDTO.unitNetTime"
				:rules="[{ required: true, message: '请输入单位净值', trigger: 'change' }]">
					<el-date-picker v-model="dataForm.prdIndexDTO.unitNetTime" type="date" format="yyyy-MM-dd" 
                    value-format="yyyy-MM-dd" placeholder="请选择时间" style="width:35%"></el-date-picker>
                ...
                <el-form-item label="累计净值：" prop="prdIndexDTO.totalNetTime"
				:rules="[{ required: true, message: '请输入累计净值', trigger: 'blur' }]">
                    <el-date-picker v-model="dataForm.prdIndexDTO.totalNetTime" type="date" format="yyyy-MM-dd" 
                    value-format="yyyy-MM-dd" placeholder="请选择与单位净值时间相同的时间" style="width:35%"></el-date-picker>
				 ...
				 <el-form-item label="三个月涨幅：" prop="prdIndexDTO.rate"
				 :rules="[{ required: true, message: '请输入三个月涨幅', trigger: 'blur' }]">
                    <el-input v-model="dataForm.prdIndexDTO.rate" type="number" min='0' placeholder="请输入三个月涨幅"></el-input>
                </el-form-item>
				 <el-form-item label="六个月涨幅：" prop="prdIndexDTO.rate2"
				 :rules="[{ required: true, message: '请输入六个月涨幅', trigger: 'blur' }]">
                    <el-input v-model="dataForm.prdIndexDTO.rate2" type="number" min='0' placeholder="请输入六个月涨幅"></el-input>
                </el-form-item>
				 <el-form-item label="一年涨幅：" prop="prdIndexDTO.rate3"
				 :rules="[{ required: true, message: '请输入一年涨幅', trigger: 'blur' }]">
			...
			<el-form-item label="支取规则：" prop="prdIndexDTO.dealRuleText">
				...
				:detailContent="dataForm.dealRuleText" @quillEditorUploaderImgSon="quillEditorUploaderImg">
				</MyQuillEditor>
			</el-form-item>
			<el-form-item label="计息规则：" prop="prdIndexDTO.rateRuleText">
				<!-- 富文本组件 -->
				<MyQuillEditor id="editor3" @onEditorChange="onEditorChangeDeal" 
				:detailContent="dataForm.rateRuleText" @quillEditorUploaderImgSon="quillEditorUploaderImg">
				</MyQuillEditor>
			</el-form-item>
...
import {forFundPage,getCurrencyAndOrg,getSeriesByPrdType,forFundList,getPrdTypeByTypeCode,getPrdSeriesById,
addFund,updateFund,deleteFund,cloneFund,getLongLinkFund,importPureDebtFund,exportPureDebtFund,importExportFund,exportExportFund } from "../../../apis/index"
...
export default {
	...
	methods: {
		...
		handleCopyLink(row){
			...
			getLongLinkFund(params).then((res) => {
		...
		uploadFile(type) {
			if (Object.keys(this.fileObj).length === 0){
				...
				//调导入接口
				if(type == 1){ //货基产品
					importExportFund(form).then((res) => {
					...
					importPureDebtFund(form).then((res) => {
		...
		handleExport(type){
			let params = ''
			if(type == 1){ //货基产品
				exportExportFund(params).then((res) => {
				...
				exportPureDebtFund(params).then((res) => {
