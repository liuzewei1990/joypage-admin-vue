  ...
  <div class="prdSeries">
	<CommonTable :isFluid="false" @height="tableHeight=$event">
		...
		<div slot="search">
			<el-form :inline="true" :size="size" :model="topCheckData" ref='topData'>
				...
				<el-form-item label="存款类型：" prop="depsoitTypeId">
					<el-select v-model="topCheckData.depsoitTypeId" placeholder="请选择" clearable
					@change='searchOrglistSelect'>
						<el-option v-for="item in allSelectData.depositTypeList"
						:key="item.value" :label="item.name" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="存款系列：" prop="prdSeriesId">
					...
					@change='depositSeriesListSelect'>
						<el-option v-for="item in allSelectData.depositSeriesList"
				...
				<el-form-item label="操作人：" prop="updId">
					...
					<el-select v-model="topCheckData.updId" placeholder="请选择操作人" filterable clearable>
						<el-option v-for="item in allSelectData.userList"
						:key="item.value" :label="item.name" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="操作时间：" prop="updDate">
					<el-date-picker v-model="topCheckData.updDate" type="daterange" 
		...
		<div slot="action">
			...
			@click="dialogUploadVisible = true" >导入</el-button>
		...
		<!-- 表格内容栏 -->
		<el-table :height="tableHeight" size="mini" :data="prdDepositList" style="width: 100%" stripe border="true"
	...
	<el-dialog :title="!dataForm.id ? '新增' : '修改'" width="80%" :visible.sync="dialogVisible" :before-close="closeDialog">
		<el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size"
			...
			<el-form-item label="存款类型：" prop="depsoitTypeId">
				<el-select v-model="dataForm.depsoitTypeId" placeholder="请选择" clearable
				@change='depositTypeListSelect' :disabled="dataForm.id">
					<el-option v-for="item in allSelectData.depositTypeList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
			...
			<el-form-item label="产品区域：" prop="prdIndex.nationalType">
				<el-select v-model="dataForm.prdIndex.nationalType" placeholder="请选择" clearable
			...
			<el-form-item label="机构名称：" prop="prdIndex.orgId">
				<!-- filterable开启可搜索模式  -->
				<el-select v-model="dataForm.prdIndex.orgId" placeholder="选择产品区域后，再选择机构名称" filterable clearable
			...
			<el-form-item label="存款系列：" prop="prdIndex.prdSeriesId">
				<el-select v-model="dataForm.prdIndex.prdSeriesId" placeholder="请选择类型和机构名称后，再选择系列" clearable
				@change='depositSeriesListSelect'>
					<el-option v-for="item in allSelectData.depositSeriesList"
					:key="item.value" :label="item.name" :value="item.id">
					</el-option>
			...
			<el-form-item label="是否在‘行情’展示：" prop="prdIndex.hqPrd">
				<el-select  v-model="dataForm.prdIndex.hqPrd" clearable>
					<el-option v-for="item in allSelectData.hqPrdList"
					:key="item.value" :label="item.name" :value="item.value">
					</el-option>
				</el-select>
			</el-form-item>
			...
			<template v-if="dataForm.depsoitTypeId == 1 || dataForm.depsoitTypeId == 3" prop="depositRateList[0]">
				...
				<template v-if="dataForm.depsoitTypeId == 3">
					<el-form-item label="利率（%）：" prop="depositRateList[0].lowRate"
					:rules="[{ required: true,  message: '请输入利率', trigger: 'blur' }]"
					>
				...
				<!-- 活期的利率，用rate -->
				<el-form-item v-if="dataForm.depsoitTypeId == 1" label="利率（%）：" prop="depositRateList[0].rate"
				:rules="[{ required: true, message: '请输入利率', trigger: 'blur' }]">
				...
				<!-- 结构性有2个字段：期限，锁定期： -->
				<template v-if="dataForm.depsoitTypeId == 3">
					<el-form-item label="期限："  prop="depositRateList[0].lengthTime" 
					:rules="[{ required: true, message: '请输入期限', trigger: 'blur' }]">
			...
			<template v-if="dataForm.depsoitTypeId != 1 && dataForm.depsoitTypeId != 3">
				<el-form-item class="addRate" label="利率：" prop="depositRateList" 
				:rules="[{ required: true, message: '请输入利率333', trigger: 'blur' }]">
						...
						<el-table-column prop="homepageLockupPeriodStatu" :label="'榜单锁\n定期展示'" width='80px' header-align="center" align="center">
							<template slot-scope="scope">
								<el-select  v-model="scope.row.homepageLockupPeriodStatu" clearable>
									<el-option v-for="item in allSelectData.homepageLockupPeriodStatuList"
									:key="item.value" :label="item.name" :value="item.value">
									</el-option>
								</el-select>
							</template>
						...
						<el-table-column prop="prdCheck" label="产品展示" header-align="center" align="center">
							<template slot-scope="scope">
								<el-select  v-model="scope.row.prdCheck" clearable>
									<el-option v-for="item in allSelectData.prdCheckList"
									:key="item.value" :label="item.name" :value="item.value">
									</el-option>
								</el-select>
							</template>
			...
			<template v-if="dataForm.depsoitTypeId == 3">
				<el-form-item label="起售日期：" prop="sellStartDate">
					<el-date-picker v-model="dataForm.sellStartDate" type="date" placeholder="请输入起售日期"
					format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
				<el-form-item label="截止日期：" prop="sellEndDate">
					<el-date-picker v-model="dataForm.sellEndDate" type="date" placeholder="请输入截止日期"
					format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
				<el-form-item label="起息日：" prop="intereatStartDate">
					<el-date-picker v-model="dataForm.intereatStartDate" type="date" placeholder="请输入起息日"
					format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
				<el-form-item label="结息日：" prop="intereatEndDate">
					<el-date-picker v-model="dataForm.intereatEndDate" type="date" placeholder="请输入结息日"
					format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
			</template>
			...
			<template v-if="dataForm.depsoitTypeId == 5">
				...
				<el-form-item label="发行开始时间：" prop="publishStartTime">
					<el-date-picker v-model="dataForm.publishStartTime" type="date" placeholder="请选择发行开始时间"
					format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
				<el-form-item label="发行结束时间：" prop="publishEndTime">
					<el-date-picker v-model="dataForm.publishEndTime" type="date" placeholder="请选择发行结束时间"
					format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
				</el-form-item>
				...
				<el-form-item label="是否可转让：" prop="transferFlag">
					<el-select v-model="dataForm.transferFlag" placeholder="请选择" clearable>
						<el-option v-for="item in allSelectData.transferFlagList"
						:key="item.value" :label="item.name" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="是否可赎回：" prop="buyBackFlag">
					<el-select v-model="dataForm.buyBackFlag" placeholder="请选择" clearable>
						<el-option v-for="item in allSelectData.buyBackFlagList"
						:key="item.value" :label="item.name" :value="item.value">
						</el-option>
					</el-select>
				</el-form-item>
			...
			<el-form-item label="产品描述：" prop="depictTextArea">
				...
				:detailContent="dataForm.depictTextArea" @quillEditorUploaderImgSon="quillEditorUploaderImg">
				</MyQuillEditor>
			...
			<el-form-item label="支取规则：" prop="prdIndex.dealRuleText">
				...
				:detailContent="dataForm.prdIndex.dealRuleText" @quillEditorUploaderImgSon="quillEditorUploaderImg">
				</MyQuillEditor>
			...
			<el-form-item label="计息规则：" prop="prdIndex.rateRuleText">
				...
				:detailContent="dataForm.prdIndex.rateRuleText" @quillEditorUploaderImgSon="quillEditorUploaderImg">
				</MyQuillEditor>
			...
			<template v-if="dataForm.depsoitTypeId == 5">
				<el-form-item label="存入规则：" prop="depositRule">
					<!-- 富文本组件 -->
					<MyQuillEditor id="editor4" @onEditorChange="onEditorChangeDeal" 
					:detailContent="dataForm.depositRule" @quillEditorUploaderImgSon="quillEditorUploaderImg">
					</MyQuillEditor>
					<!-- <el-card style="height: 450px;">
						<el-upload
				...
				<el-form-item label="转让规则：" prop="makeOverRule">
					<!-- 富文本组件 -->
					<MyQuillEditor id="editor5" @onEditorChange="onEditorChangeDeal" 
					:detailContent="dataForm.makeOverRule" @quillEditorUploaderImgSon="quillEditorUploaderImg">
					</MyQuillEditor>
					<!-- <el-card style="height: 540px;">
						<el-upload
				...
				<el-form-item label="转出规则：" prop="turnOutRule">
					<!-- 富文本组件 -->
					<MyQuillEditor id="editor6" @onEditorChange="onEditorChangeDeal" 
					:detailContent="dataForm.turnOutRule" @quillEditorUploaderImgSon="quillEditorUploaderImg">
					</MyQuillEditor>
					<!-- <el-card style="height: 450px;">
						<el-upload
...
import {forDepositPage,getCurrencyAndOrg,getSeriesByPrdType,forDepositList,getPrdTypeByTypeCode,getPrdSeriesById,
addDeposit,updateDeposit,deleteDeposit,cloneDeposit,getLongLink,importDeposit,exportDeposit,forUpload } from "../../../apis/index"
...
export default {
	...
	methods: {
		...
		getPrdDepositSelect(){
			let params = ''
			forDepositPage(params).then(res => {
		...
		getPrdDepositList: function(type) {
			...
			forDepositList(params).then(res => {
...
<style lang="less" scoped>
	...
	.addForm{
		...
		/deep/ .el-form-item.addRate.el-form-item--small{
			/deep/ .el-form-item__label-wrap,.el-form-item__content{ //利率集合整体给margin
