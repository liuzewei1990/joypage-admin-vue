    ...
    <div class="prdSeries">
        <CommonTable :isFluid="false" @height="tableHeight = $event">
                ...
                <el-form :inline="true" :size="size" :model="topCheckData" ref="topData">
                    ...
                    <el-form-item label="理财类型：" prop="prdTypeId">
                        <el-select v-model="topCheckData.prdTypeId" placeholder="请选择" clearable @change="searchOrglistSelect">
                            <el-option v-for="item in allSelectData.financeTypeList" :key="item.value" :label="item.name" :value="item.value"> </el-option>
                        </el-select>
                    </el-form-item>
            ...
            <div slot="action">
                ...
                <el-button class="importBtn" style="float: left" :size="size" icon="el-icon-sort-down" type="primary" @click="handleImport(2)">理财导入</el-button>
                <el-button class="exportBtn" :size="size" icon="el-icon-sort-up" type="primary" plain @click="handleExport(2)">理财导出</el-button>
                <el-button class="importBtn" style="float: left" :size="size" icon="el-icon-sort-down" type="primary" @click="handleImport(5)">保险理财导入</el-button>
                <el-button class="exportBtn" :size="size" icon="el-icon-sort-up" type="primary" plain @click="handleExport(5)">保险理财导出</el-button>
                <el-button class="importBtn" style="float: left" :size="size" icon="el-icon-sort-down" type="primary" @click="handleImport(6)">投融资导入</el-button>
        ...
        <el-dialog :title="!dataForm.id ? '新增' : '修改'" width="80%" :visible.sync="dialogVisible" :before-close="closeDialog">
            <el-form :model="dataForm" label-width="auto" :rules="formRules" ref="dataForm" :size="size" label-position="left" class="addForm">
                <el-form-item label="理财类型：" prop="prdIndexDTO.prdTypeId">
                    <el-select v-model="dataForm.prdIndexDTO.prdTypeId" placeholder="请选择" clearable @change="financeTypeListSelect" :disabled="dataForm.id">
                        <el-option v-for="item in allSelectData.financeTypeList" :key="item.value" :label="item.name" :value="item.value"> </el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="理财系列：" prop="prdIndexDTO.prdSeriesId">
                    <el-select v-model="dataForm.prdIndexDTO.prdSeriesId" placeholder="请选择" clearable @change="depositSeriesListSelect">
                        <el-option v-for="item in allSelectData.prdSeriesList" :key="item.value" :label="item.name" :value="item.id"> </el-option>
                    </el-select>
                </el-form-item>
                <!-- 保险理财有： 所属基金公司名称：-->
                <template v-if="dataForm.prdIndexDTO.prdTypeId == 5">
                    <el-form-item label="所属基金公司名称：" prop="fundId">
                        <el-select v-model="dataForm.fundId" placeholder="请选择" filterable clearable @change="fundCompanyListSelect">
                ...
                <template v-if="dataForm.prdIndexDTO.prdTypeId == 5" prop="prdIndexDTO">
                    <el-form-item label="七日年化收益率（%）：" prop="prdIndexDTO.rate">
                        <el-date-picker
                            v-model="dataForm.prdIndexDTO.rateTime"
                            type="date"
                            format="yyyy-MM-dd"
                            value-format="yyyy-MM-dd"
                            placeholder="请选择七日年化收益率"
                    ...
                    <el-form-item label="万份年化收益（%）：" prop="prdIndexDTO.rate2">
                        <el-date-picker
                            v-model="dataForm.prdIndexDTO.rate2Time"
                            type="date"
                            format="yyyy-MM-dd"
                            value-format="yyyy-MM-dd"
                            placeholder="请选择万份年化收益"
                            style="width: 20%"></el-date-picker>
                ...
                <template v-if="dataForm.prdIndexDTO.prdTypeId == 5">
                    <el-form-item label="起息日：" prop="valueDate">
                        <el-date-picker v-model="dataForm.valueDate" type="date" placeholder="请选择起息日" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    </el-form-item>
                    <el-form-item label="到期日：" prop="prdEndDate">
                        <el-date-picker v-model="dataForm.prdEndDate" type="date" placeholder="请选择到期日" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    </el-form-item>
                ...
                <template v-if="dataForm.prdIndexDTO.prdTypeId != 5">
                    <el-form-item label="风险等级：" prop="riskLevel">
                        <el-select v-model="dataForm.riskLevel" placeholder="请选择">
                            <el-option v-for="item in allSelectData.riskLevelList" :key="item.value" :label="item.name" :value="item.value"> </el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="募集开始日期：" prop="collectStartDate">
                        <el-date-picker
                            v-model="dataForm.collectStartDate"
                            type="date"
                    ...
                    <el-form-item label="募集结束日期：" prop="collectEndDate">
                        <el-date-picker v-model="dataForm.collectEndDate" type="date" placeholder="请选择募集结束日期" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    </el-form-item>
                    <el-form-item label="理财开始日期：" prop="finStartDate">
                        <el-date-picker v-model="dataForm.finStartDate" type="date" placeholder="请选择理财开始日期" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    </el-form-item>
                    <el-form-item label="理财结束日期：" prop="finEndDate">
                        <el-date-picker v-model="dataForm.finEndDate" type="date" placeholder="请选择理财结束日期" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    </el-form-item>
                </template>
                ...
                <template v-if="dataForm.prdIndexDTO.prdTypeId == 5">
                    <el-form-item label="是否允许申购：" prop="isPayment">
                        <el-select v-model="dataForm.isPayment" placeholder="请选择" clearable>
                            <el-option v-for="item in allSelectData.isPaymentList" :key="item.value" :label="item.name" :value="item.value"> </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="是否允许赎回：" prop="isRedeem">
                        <el-select v-model="dataForm.isRedeem" placeholder="请选择" clearable>
                            <el-option v-for="item in allSelectData.isRedeemList" :key="item.value" :label="item.name" :value="item.value"> </el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="产品清算日：" prop="prdSettleDate">
                        <el-date-picker v-model="dataForm.prdSettleDate" type="date" placeholder="请选择产品清算日期" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
                    </el-form-item>
                ...
                <el-form-item label="支取规则：" prop="prdIndexDTO.dealRuleText">
                    <!-- 富文本组件 -->
                    <MyQuillEditor id="editor2" @onEditorChange="onEditorChangeDeal" :detailContent="dataForm.prdIndexDTO.dealRuleText" @quillEditorUploaderImgSon="quillEditorUploaderImg">
                    </MyQuillEditor>
                </el-form-item>
                <el-form-item label="计息规则：" prop="prdIndexDTO.rateRuleText">
                    <!-- 富文本组件 -->
                    <MyQuillEditor id="editor3" @onEditorChange="onEditorChangeDeal" :detailContent="dataForm.prdIndexDTO.rateRuleText" @quillEditorUploaderImgSon="quillEditorUploaderImg">
                    </MyQuillEditor>
                </el-form-item>
            ...
            return {
                ...
                formRules: {
                    ...
                    prdIndexDTO: {
                        ...
                        rate: [{ required: true, message: "请输入七日年化收益率", trigger: "blur" }],
                        rate2: [{ required: true, message: "请输入万份年化收益", trigger: "blur" }],
                        rate3: [{ required: true, message: "请输入预期年化收益", trigger: "blur" }],
                        isInterview: [{ required: true, message: "请选择是否面签", trigger: "change" }],
                        period: [{ required: true, message: "请输入期限", trigger: "blur" }]
                    },
                    period: [{ required: true, message: "请输入期限", trigger: "blur" }],
